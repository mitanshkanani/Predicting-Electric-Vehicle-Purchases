"""Local verification for ev_s6e9_v14_hybrid.py. LightGBM/XGBoost are not installed here so
...
import ev_s6e9_v14_hybrid as V
...
# ============================================================================
# 0. v14-SPECIFIC: the four things this file exists for must actually be true.
check("v14 runs 10-fold (more rows per model: the decorrelation source that carried Aadit)",
      V.N_SPLITS == 10, str(V.N_SPLITS))
check("v14 adds a CatBoost member with native categoricals",
      "cat_ref" in V.BLEND_WEIGHTS and hasattr(V, "run_catboost"), str(list(V.BLEND_WEIGHTS)))
check("v14 blend weights are FIXED and sum to 1 (never OOF-searched)",
      abs(sum(V.BLEND_WEIGHTS.values()) - 1.0) < 1e-9, str(V.BLEND_WEIGHTS))
check("v14 keeps v8's verified TE ladder and the reverted 13-anchor default",
      V.TE_SMOOTHS == ("auto", 10.0, 100.0) and V.FULL_ANCHOR_SET is False)
check("v14 keeps the fold-rank fix and the band prior",
      callable(getattr(V, "fold_pct", None)) and len(V.NON_BUYER_BANDS) == 24
      and len(V.BUYER_BANDS) == 2)
check("v14 does NOT ship the Gaussian-kernel encoder (broken on continuous keys)",
      not hasattr(V, "GammaKernelTE"))
...
check("v14 keeps bagging at 3 repeats: 10-fold models are more alike, so the gain is "
      "mostly banked by 3 and 5 would cost ~6.5h",
      len(V.REPEAT_SEEDS) == 3 and len(set(V.REPEAT_SEEDS)) == 3, str(V.REPEAT_SEEDS))
check("v14 keeps v8's verified TE ladder", V.TE_SMOOTHS == ("auto", 10.0, 100.0))
check("v14 keeps the $50/$250 fine-income bins (neutral, free)",
...
(bt, bv, bc), (wt, wv, wc), (kt, kv, kc) = V.fold_matrix(F_full, ytr, np.arange(11000), np.arange(11000, 15000), 42)
...
(_, bv2, bc2), (_, wv2, wc2), _ = V.fold_matrix(F_full, yf, np.arange(11000), np.arange(11000, 15000), 42)
...
(_, bv3, _), (_, wv3, _), _ = V.fold_matrix(F_full, yf2, np.arange(11000), np.arange(11000, 15000), 42)
...
# ============================================================================
work = os.path.join(HERE, "_v14_smoke")
...
EXPECT = 4 * len(V.REPEAT_SEEDS) * V.N_SPLITS
check("all repeats x folds x 4 members ran (CatBoost included for real)",
      txt.count("auc=") == EXPECT, f"{txt.count('auc=')} (want {EXPECT})")
files = sorted(f for f in os.listdir(work) if f.startswith("submission_v14"))
...
ck = [d for d in os.listdir(work) if d.startswith("v14_ckpt")]
...
if ck:
    ...
    check("one checkpoint per (member, repeat, fold)", nfiles == EXPECT, f"{nfiles} (want {EXPECT})")
    ...
    got_oof = np.load(os.path.join(work, "v14_lgbm_ref_oof_rank.npy"))
    got_te = np.load(os.path.join(work, "v14_lgbm_ref_test.npy"))
...
def gate_aucs(t):
    ...
    return out
g = gate_aucs(txt)
check("gate scored all 4 members", len(g) == 4, str(g))
...
def health(t):
    ...
    return out
h = health(txt)
check("health table covers every member",
      set(h) == {"lgbm_ref", "xgb_ref", "cat_ref", "lr"}, str(h))
...
# The stubbed members are fully independent across repeats, so their apparent bagging gain
# is absurd (+0.049) and the health check must scream. Real CatBoost is not absurd, and the
# check must stay quiet on it. Both halves matter: that it has teeth, and that it does not
# cry wolf on an honest member.
check("health check has teeth: absurd stub gains ARE flagged",
      h["lgbm_ref"][2] and h["xgb_ref"][2] and h["lr"][2], str(h))
check("health check does not false-alarm on the REAL CatBoost member",
      not h["cat_ref"][2] and 0 < h["cat_ref"][1] < 0.005,
      f"cat_ref foldrank gap {h['cat_ref'][1]:+.6f}")
check("gate still reports and still chooses an argmax member under that flag",
      len(g) == 4 and "SUBMIT" in txt)
...
# Drift must be judged as an OUTLIER, not against an absolute tolerance: the real
# CatBoost member shows a raw-pooling gap above FOLD_GAP_TOL on this 8k-row sample,
# which is the v9 effect reproduced by an honest model rather than a stub. That is the
# reason fold-ranking exists, so the meaningful assertion is that the deliberately
# injected member is an order of magnitude worse than every naturally-behaved one.
others = [abs(h2[m][0]) for m in ("xgb_ref", "cat_ref", "lr") if m in h2]
check("5b drift is member-specific: injected lgbm is a >10x outlier vs every other member",
      bool(others) and abs(h2["lgbm_ref"][0]) > 10 * max(others),
      f"lgbm {h2['lgbm_ref'][0]:+.6f} vs others max {max(others):.6f}")
check("5b real CatBoost also shows a raw-pooling gap that fold-ranking removes "
      "(why the fix is not just for stubs)",
      abs(h2["cat_ref"][0]) > V.FOLD_GAP_TOL,
      f"raw {h2['cat_ref'][0]:+.6f}")
