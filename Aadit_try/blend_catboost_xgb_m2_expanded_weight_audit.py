"""
Controlled follow-up: expand CatBoost/XGBoost m=2 rank-blend weight search.

WHY THIS EXPERIMENT
-------------------
The previous leakage-safe blend audit selected CAT weight = 0.50 on ALL 5 held
folds, but 0.50 was the LOWER BOUND of the search grid.

That means the previous search could not answer whether the true optimum prefers
LESS CatBoost / MORE XGBoost.

HYPOTHESIS
----------
Allowing CAT weight below 0.50 may improve the leakage-safe meta-CV blend.

ONLY CHANGE
-----------
OLD weight grid:
    CAT 0.50 -> 1.00 by 0.01

NEW weight grid:
    CAT 0.00 -> 1.00 by 0.01

HELD FIXED
----------
- exact same frozen 5 folds
- exact same CatBoost 3-seed OOF/test predictions
- exact same XGBoost exact-TE m=2 OOF/test predictions
- rank blending
- for each held fold, choose the weight on the OTHER FOUR folds only
- same deployment convention: mean selected CAT weight, rounded to 0.01 grid
- no Kaggle leaderboard information is used

CONTROL
-------
Before running the expanded search, reproduce the prior restricted-grid result:

    meta-CV OOF = 0.94566049
    selected CAT weights = [0.50, 0.50, 0.50, 0.50, 0.50]

If this does not reproduce, STOP.

Run:
    python blend_catboost_xgb_m2_expanded_weight_audit.py
"""
...
EXPECTED_RESTRICTED_META_AUC = 0.94566049
EXPECTED_RESTRICTED_WEIGHTS = [0.50, 0.50, 0.50, 0.50, 0.50]

RESTRICTED_GRID = np.round(np.arange(0.50, 1.0001, 0.01), 2)
...
def main() -> None:
    ...
    print("=" * 88)
    print("CATBOOST + XGBOOST m=2 EXPANDED WEIGHT AUDIT")
    print("=" * 88)
    print(f"Frozen fold SHA256 verified : {fold_hash}")
    print(f"CatBoost 3-seed OOF         : {cat_auc:.8f}")
    print(f"XGBoost exact-TE m=2 OOF    : {xgb_auc:.8f}")
    print()

    # ---------------------------------------------------------
    # CONTROL: reproduce the previous 0.50 -> 1.00 audit first.
    # ---------------------------------------------------------
    print("CONTROL: reproducing prior restricted CAT-weight grid 0.50 -> 1.00...")
    ...
    print(
        f"Reproduced restricted meta-CV : "
        f"{restricted_meta_auc:.8f}"
    )
    print(
        f"Expected restricted meta-CV   : "
        f"{EXPECTED_RESTRICTED_META_AUC:.8f}"
    )
    print(
        f"Reproduced restricted weights : "
        f"{restricted_weights}"
    )
    print(
        f"Expected restricted weights   : "
    ...
    restricted_folds.to_csv(
        args.output_dir
        / "restricted_control_fold_metrics.csv",
        index=False,
    )
    ...
    if not (
        ...
        raise RuntimeError(
            "STOP: restricted-grid control did not reproduce. "
            "Do not trust the expanded audit. Send the terminal output "
            "and restricted_control_fold_metrics.csv so this SAME file "
            "can be fixed."
        )
    ...
    # ---------------------------------------------------------
    print("NEW AUDIT: expanded CAT-weight grid 0.00 -> 1.00...")
    ...
    print()
    print(
        f"Expanded meta-CV OOF         : "
        f"{expanded_meta_auc:.8f}"
    )
    print(
        f"Restricted meta-CV OOF       : "
        f"{restricted_meta_auc:.8f}"
    )
    print(
        f"Delta vs restricted blend    : "
        f"{delta_vs_restricted:+.8f}"
    )
    print(
        f"Delta vs XGB m=2 single      : "
        f"{delta_vs_xgb:+.8f}"
    )
    print(
    ...
    if (
        ...
        decision = "NO_CLEAR_BLEND_GAIN_OVER_XGB"
    elif (
        expanded_meta_auc
        <= xgb_auc
    ):
    ...
    summary = [
        "EXPERIMENT: CATBOOST + XGBOOST m=2 EXPANDED WEIGHT AUDIT",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "The previous blend selected CAT=0.50 on every held fold, but 0.50",
        "was the lower boundary. Test whether less CatBoost / more XGBoost",
        "improves leakage-safe meta-CV.",
        "",
        "ONLY CHANGE",
        "CAT weight grid: 0.50..1.00 -> 0.00..1.00 by 0.01",
        "",
        "CONTROL",
        f"Restricted meta-CV expected: {EXPECTED_RESTRICTED_META_AUC:.8f}",
        f"Restricted meta-CV reproduced: {restricted_meta_auc:.8f}",
        f"Restricted weights reproduced: {restricted_weights}",
        "CONTROL: PASS",
        "",
        "SINGLE MODELS",
        f"CatBoost 3-seed OOF: {cat_auc:.8f}",
        f"XGBoost m=2 OOF: {xgb_auc:.8f}",
        "",
        "EXPANDED META-CV",
        f"Meta-CV OOF: {expanded_meta_auc:.8f}",
        f"Delta vs restricted blend: {delta_vs_restricted:+.8f}",
        f"Delta vs XGB m=2 single: {delta_vs_xgb:+.8f}",
        f"Folds blend > CatBoost: {folds_blend_beats_cat}/5",
        f"Folds blend > XGB m=2: {folds_blend_beats_xgb}/5",
        ...
        f"Expanded selections hit a search boundary: {boundary_note}",
        "",
        f"DECISION: {decision}",
        "",
    ...
    print("  2. summary.txt")
    print("  3. restricted_control_fold_metrics.csv")
    print("  4. expanded_fold_metrics.csv")
