"""
xgboost_fine_income_lossguide_gpu.py

Kaggle Playground Series S6E9

Controlled structural experiment:
Keep the newly validated fine-grained income representation and replace ONLY the
XGBoost training configuration with one fixed high-resolution lossguide recipe.

HYPOTHESIS
----------
The current depth-4 XGBoost now has strong representation:
- exact income + commute Bayesian TE
...
The next bottleneck may be tree construction rather than feature representation.
A high-resolution histogram + lossguide recipe may exploit sharp local structure
more effectively than the current XGBoost configuration.

ONLY CHANGE
-----------
Replace the current XGBoost parameter recipe with ONE fixed structural recipe:
...
BASELINE
--------
Current fine-income XGB:
    OOF AUC = 0.94606664

Run:
    python xgboost_fine_income_lossguide_gpu.py
"""
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_fine_income_lossguide_gpu"
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Fine-income XGB with one fixed high-resolution lossguide recipe."
        )
    )
...
def main() -> None:
    ...
    if abs(
        ...
        raise ValueError(
            ...
            "hierarchical-commute XGB champion.\n"
    ...
    print("=" * 96)
    print("XGBOOST + FINE INCOME TE + HIGH-RESOLUTION LOSSGUIDE")
    print("=" * 96)
    ...
    print("HYPOTHESIS:")
    print(
        "  With the new $50/$250 income signal retained, tree construction may now "
        "be the bottleneck. Test one fixed high-resolution lossguide recipe."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Replace the XGBoost training configuration with the fixed "
        "high-resolution lossguide recipe. No parameter sweep."
    )
    print()
    print("HELD FIXED:")
    ...
    for outer_fold in range(5):
        ...
        # ONE fixed architecture recipe. No tuning loop.
        # All feature engineering, folds, margins, and target encodings remain fixed.
        model.set_params(
    ...
    if (
        ...
        decision = (
            "KEEP_HIGH_RES_LOSSGUIDE"
        )
    else:
        decision = (
            "REJECT_HIGH_RES_LOSSGUIDE"
        )
    ...
    summary_lines = [
        "EXPERIMENT: XGBOOST + FINE INCOME TE + HIGH-RESOLUTION LOSSGUIDE",
        "=" * 84,
        "",
        "HYPOTHESIS",
        "Can a fixed high-resolution lossguide XGBoost recipe exploit the now-",
        "validated fine-income representation better than the current model?",
        "",
        "ONLY CHANGE",
        "Replace only the XGBoost parameter recipe with the fixed high-resolution lossguide configuration.",
        "",
        "HELD FIXED",
        f"- frozen fold SHA256: {fold_hash}",
        "- current hierarchical-income + hierarchical-commute XGB representation",
        ...
        "- CUDA GPU",
        "",
        "FIXED CANDIDATE XGBOOST RECIPE",
        "- max_bin=1024",
        "- max_leaves=512",
        "- grow_policy=lossguide",
        "- learning_rate=0.01",
        ...
        f"High-resolution lossguide candidate OOF: {candidate_auc:.8f}",
    ...
    print()
    print("=" * 96)
    print("HIGH-RESOLUTION LOSSGUIDE EXPERIMENT COMPLETE")
    print("=" * 96)
    print(f"Fine-income XGB baseline : {baseline_auc:.8f}")
    ...
    print()
    print("Fine income TE importances:")
    if len(
        fine_income_importance
    ):
        print(
