...
Controlled experiment — use the original-data EV buying recipe as an XGBoost base margin on top of the validated income-digit m=2 model.
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + INCOME DIGITS + RECIPE BASE-MARGIN EXPERIMENT")
    print("=" * 88)
    ...
    # Keep the already-validated income-digit representation exactly.
    X_candidate_base, X_candidate_test_base, digit_features = (
    ...
    # initialize XGBoost from the original-data recipe's implied log-odds.
    (
        train_recipe_margin,
        train_recipe_score,
    ...
    print("HYPOTHESIS:")
    print(
        "  The original-dataset recipe hurt when added as an ordinary feature, "
        "but it may still help as an XGBoost base margin: the trees start from "
        "the recipe prediction and learn only residual corrections."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Keep the validated income-digit model features unchanged and provide "
        "the recipe-implied log-odds as base_margin during train/eval/inference."
    )
    print()
    print("HELD FIXED:")
    ...
    for outer_fold in range(5):
        ...
        print(
            ...
            f"+base_margin={candidate_fold_auc:.8f} "
    ...
    digit_importance.to_csv(
        args.output_dir / "income_digit_feature_importance.csv",
        index=False,
    )
    ...
    if (
        ...
        decision = "KEEP_RECIPE_BASE_MARGIN"
    else:
        decision = "REJECT_RECIPE_BASE_MARGIN"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + INCOME DIGITS + RECIPE BASE MARGIN",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Can the original-dataset recipe help the validated income-digit XGBoost",
        "when used as a base margin rather than as an ordinary feature?",
        "",
        "ONLY CHANGE",
        "Recipe-implied log-odds supplied as XGBoost base_margin.",
        "",
        "HELD FIXED",
        "- frozen 5 folds",
        ...
        f"Income digits + recipe base-margin OOF: {candidate_auc:.8f}",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            ...
            f"+base_margin={row.digit_candidate_auc:.8f} "
    ...
    summary_lines.extend(
        [
            "",
            "INCOME DIGIT FEATURE IMPORTANCE (BASE-MARGIN MODEL)",
        ]
    )

    for row in digit_importance.itertuples():
        summary_lines.append(
            f"{row.feature}: "
    ...
    print()
    print("=" * 88)
    print("INCOME DIGITS + RECIPE BASE-MARGIN EXPERIMENT COMPLETE")
    print("=" * 88)
    print(f"XGB m=2 baseline       : {m2_baseline_auc:.8f}")
    print(f"Income-digit baseline  : {income_digit_baseline_auc:.8f}")
    print(f"Recipe-only AUC         : {recipe_only_auc:.8f}")
    print(f"+ recipe base margin    : {candidate_auc:.8f}")
    ...
    print()
    print("Digit feature importances:")
    if len(digit_importance):
        print(
            digit_importance[
                [
                    "feature",
    ...
    print("  4. recipe_margin_diagnostics.csv")
