...
Controlled experiment — add one leakage-safe exact Annual_Income_USD × Subsidy_Available Bayesian target encoding on top of the current XGBoost champion.
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_m2_income_subsidy_interaction_te_gpu"
)
...
def build_income_subsidy_interaction_te_for_outer_fold(
    train: pd.DataFrame,
    test: pd.DataFrame,
    y: np.ndarray,
    fold_ids: np.ndarray,
    ...
    """
    Leakage-safe Bayesian target encoding for exactly ONE interaction:

        Annual_Income_USD exact identity × Subsidy_Available

    Outer-training rows receive nested OOF encodings across the remaining
    frozen folds. Outer-validation and test rows use a mapping fitted only on
    the outer-training rows.

    No validation target is used to construct its own encoding.
    """
    outer_train_mask = fold_ids != outer_fold
    outer_valid_mask = fold_ids == outer_fold

    outer_train_idx = np.flatnonzero(outer_train_mask)
    outer_valid_idx = np.flatnonzero(outer_valid_mask)

    outer_train_original_folds = fold_ids[outer_train_idx]
    outer_train_y = y[outer_train_idx]

    def make_key(df: pd.DataFrame) -> pd.Series:
        income = exact_key(
            df["Annual_Income_USD"].reset_index(drop=True),
            "Annual_Income_USD",
        )
        ...
        allowed = {"yes", "no"}
        observed = set(subsidy.unique().tolist())
        if not observed.issubset(allowed):
            raise ValueError(
                "Unexpected Subsidy_Available labels in interaction TE: "
                f"{sorted(observed)}"
            )
    ...
    outer_train_keys = make_key(
        train.iloc[outer_train_idx]
    )
    valid_keys = make_key(
        train.iloc[outer_valid_idx]
    )
    test_keys = make_key(test)

    te_name = "TE__Annual_Income_USD_exact_x_Subsidy_Available"
    ...
    # Nested OOF encoding for outer-training rows.
    for inner_valid_fold in sorted(
        np.unique(
            outer_train_original_folds
        ).tolist()
    ...
    if np.isnan(train_encoded).any():
        raise RuntimeError(
            "Income×subsidy interaction TE contains NaN values."
        )
    ...
    train_te = pd.DataFrame(
        {
            te_name: train_encoded,
        }
    )
    valid_te = pd.DataFrame(
        {
            te_name: valid_encoded,
        }
    )
    test_te = pd.DataFrame(
        {
            te_name: test_encoded,
        }
    )
        ...
        [
            {
                ...
                "median_rows_per_key_outer_train": float(
                    outer_train_keys.value_counts().median()
                ),
                "mean_rows_per_key_outer_train": float(
                    outer_train_keys.value_counts().mean()
                ),
            }
        ]
    )
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + INCOME×SUBSIDY EXACT-INTERACTION TE EXPERIMENT")
    print("=" * 88)
    ...
    # Keep the already-validated recipe base margin exactly.
    (
        train_recipe_margin,
        train_recipe_score,
    ...
    print(f"Recipe-only AUC         : {recipe_only_auc:.8f}")
    print()
    print("HYPOTHESIS:")
    print(
        "  Exact income values repeat enough to support leakage-safe local target "
        "statistics. Because subsidy is one of the strongest original recipe "
        "terms, the target rate for the SAME income may differ materially by "
        "subsidy status. A conditional exact-income TE may expose that structure."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Add ONE nested Bayesian target-encoding feature:"
    )
    print(
        "    - exact Annual_Income_USD identity × Subsidy_Available"
    )
    print()
    print("HELD FIXED:")
    ...
    print("  - validated recipe base margin")
    print("  - validated exact-frequency features")
    ...
    for outer_fold in range(5):
        ...
        (
            interaction_train_te,
            interaction_valid_te,
            interaction_test_te,
            interaction_diagnostics,
        ) = build_income_subsidy_interaction_te_for_outer_fold(
            train=train,
            test=test,
            y=y,
            fold_ids=fold_ids,
        ...
        for c in interaction_train_te.columns:
            train_te[c] = interaction_train_te[c].to_numpy(dtype=np.float32)
            valid_te[c] = interaction_valid_te[c].to_numpy(dtype=np.float32)
            test_te[c] = interaction_test_te[c].to_numpy(dtype=np.float32)

        te_seconds = time.perf_counter() - te_start
        diagnostic_frames.append(diagnostics)
        diagnostic_frames.append(interaction_diagnostics)
        ...
        model.fit(
            X_train,
            y[train_idx],
            base_margin=train_recipe_margin[train_idx],
            eval_set=[
                (
                    X_valid,
                    y[valid_idx],
        ...
        valid_pred = model.predict_proba(
            X_valid,
            base_margin=train_recipe_margin[valid_idx],
        )[:, 1]

        test_pred = model.predict_proba(
            X_test,
            base_margin=test_recipe_margin,
        )[:, 1]
        ...
        fold_rows.append(
            {
                "fold": outer_fold,
                "exact_frequency_baseline_auc": baseline_fold_auc,
            ...
            importance_rows.append(
                {
                    ...
                    "is_income_subsidy_interaction_te": (
                        feature
                        == "TE__Annual_Income_USD_exact_x_Subsidy_Available"
                    ),
                    "is_exact_te_feature": (
        ...
        print(
            f"Fold {outer_fold}: "
            f"exact_frequency={baseline_fold_auc:.8f} -> "
            f"+income_x_subsidy_te={candidate_fold_auc:.8f} "
    ...
    folds_improved_vs_exact_frequency = int(
        (
            fold_metrics[
                "delta_vs_exact_frequency"
            ] > 0
    ...
    folds_worse_vs_exact_frequency = int(
        (
            fold_metrics[
                "delta_vs_exact_frequency"
            ] < 0
    ...
    interaction_importance.to_csv(
        args.output_dir / "income_subsidy_interaction_te_importance.csv",
        index=False,
    )
    ...
    diagnostics[
        ...
        args.output_dir / "income_subsidy_interaction_te_diagnostics.csv",
        index=False,
    )
    ...
    if (
        delta_vs_exact_frequency > 0
        and folds_improved_vs_exact_frequency >= 3
    ):
        decision = "KEEP_INCOME_X_SUBSIDY_INTERACTION_TE"
    else:
        decision = "REJECT_INCOME_X_SUBSIDY_INTERACTION_TE"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + INCOME X SUBSIDY EXACT INTERACTION TE",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Does leakage-safe exact Annual_Income_USD × Subsidy_Available target",
        "encoding add conditional purchase signal on top of the current champion?",
        "",
        "ONLY CHANGE",
        "Added ONE nested Bayesian TE feature:",
        "TE__Annual_Income_USD_exact_x_Subsidy_Available",
        "",
        "HELD FIXED",
        "- frozen 5 folds",
        ...
        f"Recipe-only AUC: {recipe_only_auc:.8f}",
        ...
        f"Folds improved vs exact-frequency baseline: {folds_improved_vs_exact_frequency}/5",
        f"Folds worse vs exact-frequency baseline: {folds_worse_vs_exact_frequency}/5",
        "",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            f"Fold {row.fold}: "
            f"exact_frequency={row.exact_frequency_baseline_auc:.8f} -> "
            f"+income_x_subsidy_te={row.digit_candidate_auc:.8f} "
            f"({row.delta_vs_exact_frequency:+.8f})"
        )

    summary_lines.extend(
        [
            "",
            "INCOME×SUBSIDY INTERACTION-TE FEATURE IMPORTANCE",
        ]
    )
    ...
    print()
    print("=" * 88)
    print("INCOME×SUBSIDY EXACT-INTERACTION TE EXPERIMENT COMPLETE")
    print("=" * 88)
    ...
    print(f"Recipe-only AUC        : {recipe_only_auc:.8f}")
    print(f"+ income×subsidy TE    : {candidate_auc:.8f}")
    ...
    print(f"Folds improved         : {folds_improved_vs_exact_frequency}/5")
    print(f"Folds worse            : {folds_worse_vs_exact_frequency}/5")
    print(f"Decision         : {decision}")
    print(f"Runtime          : {total_seconds:.2f}s")
    print(f"Artifacts        : {args.output_dir.resolve()}")
    print()
    print("Income×subsidy interaction-TE importance:")
    if len(interaction_importance):
        print(
            interaction_importance[
                [
                    "feature",
    ...
    print("  4. income_subsidy_interaction_te_diagnostics.csv")
