...
Controlled experiment — add categorical copies of low-cardinality discrete numeric variables on top of the validated income-digit XGBoost m=2 representation.
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_m2_income_digits_discrete_numeric_copies_gpu"
)
...
def add_discrete_numeric_categorical_copies(
    X_train: pd.DataFrame,
    X_test: pd.DataFrame,
    train_source: pd.DataFrame,
    test_source: pd.DataFrame,
    ...
    """
    Keep the original numeric columns and add categorical copies of the
    low-cardinality discrete numeric features.

    This is target-free. Categories are defined jointly from train + test
    values only so XGBoost sees a consistent category vocabulary.
    """
    X_train = X_train.copy()
    X_test = X_test.copy()
    added = []
    ...
    for feature in DISCRETE_NUMERIC_FEATURES:
        if feature not in train_source.columns or feature not in test_source.columns:
            raise ValueError(f"Missing discrete numeric feature: {feature}")

        copy_name = f"FE__{feature}__cat"

        train_values = train_source[feature].astype(str)
        test_values = test_source[feature].astype(str)
        ...
        X_train[copy_name] = train_values.astype(dtype)
        X_test[copy_name] = test_values.astype(dtype)
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + INCOME DIGITS + DISCRETE-NUMERIC CATEGORICAL COPIES")
    print("=" * 88)
    ...
    # duplicate low-cardinality discrete numeric columns as categoricals,
    # while retaining their original numeric versions.
    (
        X_candidate_base,
        X_candidate_test_base,
        discrete_cat_features,
    ) = add_discrete_numeric_categorical_copies(
    ...
    print("HYPOTHESIS:")
    print(
        "  Several numeric columns are actually low-cardinality discrete variables. "
        "Keeping their numeric versions while adding categorical copies may let "
        "XGBoost model arbitrary level-specific effects without losing ordering."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Keep the validated income-digit representation and add categorical "
        "copies of these discrete numeric features:"
    )
    for feature in discrete_cat_features:
        print(f"    - {feature}")
    print()
    print("HELD FIXED:")
    ...
    for outer_fold in range(5):
            ...
            importance_rows.append(
                {
                    ...
                    "is_discrete_cat_copy": (
                        feature in discrete_cat_features
                    ),
                    "is_exact_te_feature": (
        ...
        print(
            ...
            f"+discrete_cats={candidate_fold_auc:.8f} "
    ...
    discrete_cat_importance.to_csv(
        args.output_dir / "discrete_categorical_copy_importance.csv",
        index=False,
    )
    ...
    if (
        ...
        decision = "KEEP_DISCRETE_NUMERIC_CATEGORICAL_COPIES"
    else:
        decision = "REJECT_DISCRETE_NUMERIC_CATEGORICAL_COPIES"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + INCOME DIGITS + DISCRETE-NUMERIC CATEGORICAL COPIES",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Can categorical copies of low-cardinality discrete numeric variables",
        "add level-specific signal on top of the validated income-digit XGBoost?",
        "",
        "ONLY CHANGE",
        "Added categorical copies while keeping original numeric columns:",
        *[
            f"- {feature}"
            for feature in discrete_cat_features
        ],
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            ...
            f"+discrete_cats={row.digit_candidate_auc:.8f} "
    ...
    summary_lines.extend(
        [
            "",
            "DISCRETE CATEGORICAL-COPY FEATURE IMPORTANCE",
        ]
    )

    for row in discrete_cat_importance.itertuples():
        summary_lines.append(
            f"{row.feature}: "
    ...
    print()
    print("=" * 88)
    print("INCOME DIGITS + DISCRETE-NUMERIC CATEGORICAL-COPY EXPERIMENT COMPLETE")
    print("=" * 88)
    print(f"XGB m=2 baseline       : {m2_baseline_auc:.8f}")
    print(f"Income-digit baseline  : {income_digit_baseline_auc:.8f}")
    print(f"+ discrete cat copies  : {candidate_auc:.8f}")
    ...
    print()
    print("Discrete categorical-copy importances:")
    if len(discrete_cat_importance):
        print(
            discrete_cat_importance[
                [
                    "feature",
    ...
    print("  4. discrete_categorical_copy_importance.csv")
