"""
Controlled blend audit: CatBoost 3-seed rank ensemble + income-digit XGBoost.

HYPOTHESIS
----------
The validated income-digit XGBoost is stronger than plain XGBoost m=2 and is
slightly less rank-correlated with CatBoost. Replacing the m=2 XGBoost component
may therefore improve the leakage-safe CatBoost/XGBoost blend.

CONTROL
-------
Before testing the new component, reproduce the current locally validated
CatBoost + XGBoost m=2 expanded-grid blend:

    meta-CV OOF = 0.94566717
    held-fold CAT weights = [0.37, 0.39, 0.38, 0.36, 0.41]
...
NEW:
    XGBoost exact-value Bayesian TE m=2
    + validated Annual_Income_USD digit decomposition
...
Run:
    python blend_catboost_income_digit_xgb_audit.py
"""
...
EXPECTED_RESTRICTED_META_AUC = 0.94566717
EXPECTED_RESTRICTED_WEIGHTS = [0.37, 0.39, 0.38, 0.36, 0.41]
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "blend_catboost_income_digit_xgb_audit"
)
...
def main() -> None:
    ...
    print("=" * 88)
    print("CATBOOST + INCOME-DIGIT XGBOOST LEAKAGE-SAFE RANK-BLEND AUDIT")
    print("=" * 88)
    print(f"Frozen fold SHA256 verified : {fold_hash}")
    print(f"CatBoost 3-seed OOF         : {cat_auc:.8f}")
    print(f"Income-digit XGBoost OOF    : {xgb_auc:.8f}")
    print()

    # ---------------------------------------------------------
    # CONTROL: reproduce the previous 0.50 -> 1.00 audit first.
    # ---------------------------------------------------------
    print("CONTROL: reproducing current CatBoost + XGBoost m=2 blend...")
    ...
    if not (
        ...
        raise RuntimeError(
            "STOP: current m=2 blend control did not reproduce. "
    ...
    # ---------------------------------------------------------
    print("NEW AUDIT: CatBoost + income-digit XGBoost, CAT grid 0.00 -> 1.00...")
    ...
    print(
        f"Delta vs current m=2 blend    : "
        f"{delta_vs_restricted:+.8f}"
    )
    print(
        f"Delta vs income-digit XGB    : "
        f"{delta_vs_xgb:+.8f}"
    )
    print(
    ...
    print(
        f"Folds blend > income-digit XGB        : "
    ...
    expanded_folds.to_csv(
        args.output_dir
        / "income_digit_blend_fold_metrics.csv",
        index=False,
    )
    ...
    if (
        ...
        decision = "KEEP_INCOME_DIGIT_BLEND"
    elif (
        abs(
            expanded_meta_auc
            - xgb_auc
        ...
        decision = "NO_CLEAR_BLEND_GAIN_OVER_INCOME_DIGIT_XGB"
    elif (
        expanded_meta_auc
        <= xgb_auc
    ):
        decision = "KEEP_INCOME_DIGIT_XGB_SINGLE_FOR_NOW"
    else:
    ...
    summary = [
        "EXPERIMENT: CATBOOST + INCOME-DIGIT XGBOOST RANK-BLEND AUDIT",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Does replacing plain m=2 XGBoost with the stronger validated income-digit",
        "XGBoost improve the leakage-safe CatBoost/XGBoost rank ensemble?",
        "",
        "ONLY CHANGE",
        "XGBoost component: plain m=2 -> m=2 + income digit decomposition",
        "",
        "CONTROL",
        f"Control m=2 meta-CV expected: {EXPECTED_RESTRICTED_META_AUC:.8f}",
        f"Control m=2 meta-CV reproduced: {restricted_meta_auc:.8f}",
        f"Control m=2 weights reproduced: {restricted_weights}",
        "CONTROL: PASS",
        "",
        "SINGLE MODELS",
        f"CatBoost 3-seed OOF: {cat_auc:.8f}",
        f"Income-digit XGBoost OOF: {xgb_auc:.8f}",
        "",
        "NEW META-CV BLEND",
        f"Meta-CV OOF: {expanded_meta_auc:.8f}",
        f"Delta vs current m=2 blend: {delta_vs_restricted:+.8f}",
        f"Delta vs income-digit XGB single: {delta_vs_xgb:+.8f}",
        f"Folds blend > CatBoost: {folds_blend_beats_cat}/5",
        f"Folds blend > income-digit XGB: {folds_blend_beats_xgb}/5",
    ...
    print("  2. summary.txt")
    print("  3. m2_control_fold_metrics.csv")
    print("  4. income_digit_blend_fold_metrics.csv")
