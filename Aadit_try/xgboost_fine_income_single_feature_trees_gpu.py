"""
xgboost_fine_income_single_feature_trees_gpu.py

Kaggle Playground Series S6E9

STRUCTURAL MODEL EXPERIMENT
---------------------------
Keep the COMPLETE current fine-income XGBoost feature pipeline unchanged, but
forbid XGBoost from combining different columns inside a tree.
...
HYPOTHESIS
----------
The competition appears unusually additive: the strongest known target
structure is dominated by income, environmental concern, subsidy, and range
anxiety, while many manually-created cross-feature interactions have failed.

Recent public S6E9 experimentation also reported a large gain from forbidding
a boosted tree model from combining columns within trees.

Our current pipeline already provides rich UNIVARIATE representations:
- raw features
- income digits
- exact income/commute frequency
...
If the residual relationship is mostly additive, allowing arbitrary
cross-feature interactions can spend capacity fitting noise.

ONLY CHANGE
-----------
Set XGBoost interaction_constraints so every feature is its own singleton
interaction group:

    [[feature_0], [feature_1], ..., [feature_n]]

This means a single tree path may use only one feature. The boosted ensemble
can still add effects learned from many different features across trees.

HELD FIXED
----------
- current fine-income representation
- frozen 5 folds + SHA256
- all nested/OOF target encodings
- smoothing m=2
- nested learned logistic base margin
- depth-4 XGBoost parameter recipe
- max_bin=256
- seed 42
- CUDA GPU
- early stopping
...
Run:
    python xgboost_fine_income_single_feature_trees_gpu.py
"""
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_fine_income_single_feature_trees_gpu"
)
...
def main() -> None:
    ...
    print("=" * 100)
    print("XGBOOST FINE-INCOME + SINGLE-FEATURE TREE CONSTRAINTS")
    print("=" * 100)
    ...
    print("HYPOTHESIS:")
    print(
        "  The residual target function may be mostly additive. "
        "Arbitrary cross-feature interactions may therefore fit noise."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Every feature is placed in its own singleton XGBoost "
        "interaction-constraint group."
    )
    print()
    print("HELD FIXED:")
    print("  - complete current fine-income feature representation")
    ...
    for outer_fold in range(5):
        ...
        # ONLY MODEL CHANGE:
        # each feature is its own permitted interaction group.
        # Therefore a tree cannot combine different columns on one path.
        ...
        if outer_fold == 0:
            print(
                f"Singleton interaction groups: "
                f"{len(singleton_constraints)} "
                f"(one per model feature)"
            )
    ...
    if (
        ...
        decision = (
            "POSITIVE_SINGLE_FEATURE_TREE_SIGNAL"
        )
    elif (
        delta_vs_baseline < 0
        or folds_worse >= 4
        ...
        decision = (
            "NEGATIVE_SINGLE_FEATURE_TREE_SIGNAL"
        )
    else:
        decision = (
            "WEAK_OR_INCONSISTENT_SINGLE_FEATURE_TREE_SIGNAL"
        )
    ...
    summary_lines = [
        "EXPERIMENT: XGBOOST FINE-INCOME + SINGLE-FEATURE TREE CONSTRAINTS",
        "=" * 84,
        "",
        "HYPOTHESIS",
        ...
        "",
        "ONLY CHANGE",
        "XGBoost interaction constraints: one singleton group per feature.",
        "",
        "HELD FIXED",
        ...
        "",
        "ONLY MODEL CHANGE",
        "- singleton interaction constraint per feature",
        "",
        "RESULTS",
        f"Current fine-income XGB baseline OOF: {baseline_auc:.8f}",
        f"Single-feature-tree candidate OOF: {candidate_auc:.8f}",
    ...
    print()
    print("=" * 96)
    print("SINGLE-FEATURE TREE CONSTRAINT EXPERIMENT COMPLETE")
    print("=" * 96)
