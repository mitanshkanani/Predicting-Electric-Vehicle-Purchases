...
Controlled experiment — add exact income/commute frequency-density features on top of the validated income-digit + recipe-base-margin XGBoost m=2 model.
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + INCOME DIGITS + RECIPE MARGIN + EXACT-FREQUENCY EXPERIMENT")
    print("=" * 88)
    ...
    print("HYPOTHESIS:")
    print(
        "  Exact income/commute identities are already valuable. Their occurrence "
        "frequency may expose generator-density and clipping structure that raw "
        "values, exact TE, income digits, and the recipe margin do not encode."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Add target-free train+test exact-value frequency representations for "
        "Annual_Income_USD and Daily_Commute_km."
    )
    for feature in exact_frequency_features:
        print(f"    - {feature}")
    print()
    print("HELD FIXED:")
    ...
    for outer_fold in range(5):
        ...
        fold_rows.append(
            {
                "fold": outer_fold,
                "recipe_margin_baseline_auc": baseline_fold_auc,
        ...
        print(
            f"Fold {outer_fold}: "
            f"recipe_margin={baseline_fold_auc:.8f} -> "
            f"+frequency={candidate_fold_auc:.8f} "
    ...
    folds_improved_vs_recipe_margin = int(
        (
            fold_metrics[
                "delta_vs_recipe_margin"
            ] > 0
    ...
    folds_worse_vs_recipe_margin = int(
        (
            fold_metrics[
                "delta_vs_recipe_margin"
            ] < 0
    ...
    if (
        delta_vs_recipe_margin > 0
        and folds_improved_vs_recipe_margin >= 3
    ):
        decision = "KEEP_EXACT_FREQUENCY_FEATURES"
    else:
        decision = "REJECT_EXACT_FREQUENCY_FEATURES"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + INCOME DIGITS + RECIPE MARGIN + EXACT FREQUENCY",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Can exact-value occurrence frequencies for income and commute add",
        "generator-density signal on top of the validated recipe-margin XGBoost?",
        "",
        "ONLY CHANGE",
        "Added target-free train+test exact-value frequency count/log/rate features",
        "for Annual_Income_USD and Daily_Commute_km.",
        "",
        "HELD FIXED",
        "- frozen 5 folds",
        ...
        f"Recipe margin + exact frequency OOF: {candidate_auc:.8f}",
        ...
        f"Folds improved vs recipe margin: {folds_improved_vs_recipe_margin}/5",
        f"Folds worse vs recipe margin: {folds_worse_vs_recipe_margin}/5",
        "",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            f"Fold {row.fold}: "
            f"recipe_margin={row.recipe_margin_baseline_auc:.8f} -> "
            f"+frequency={row.digit_candidate_auc:.8f} "
            f"({row.delta_vs_recipe_margin:+.8f})"
        )

    summary_lines.extend(
        [
            "",
            "EXACT-FREQUENCY FEATURE IMPORTANCE",
        ]
    )
    ...
    print()
    print("=" * 88)
    print("INCOME DIGITS + RECIPE MARGIN + EXACT-FREQUENCY EXPERIMENT COMPLETE")
    print("=" * 88)
    ...
    print(f"Recipe-only AUC        : {recipe_only_auc:.8f}")
    print(f"+ exact frequencies    : {candidate_auc:.8f}")
    ...
    print(f"Folds improved         : {folds_improved_vs_recipe_margin}/5")
    print(f"Folds worse            : {folds_worse_vs_recipe_margin}/5")
    ...
    print("  4. exact_frequency_feature_importance.csv")
