...
Controlled experiment — add one source-informed buying-recipe score on top of the validated income-digit XGBoost m=2 representation.
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_m2_income_digits_plus_source_recipe_score_gpu"
)
...
def add_source_recipe_score(
    ...
    """
    Add ONE target-free, source-informed score feature.

    Public EDA on the ORIGINAL EV-adoption dataset reported an approximate
    ...
    We do NOT assume that recipe is the competition target function. Kaggle
    states train/test are synthetic and only inspired by the original dataset.
    This experiment asks whether the original recipe is a useful representation
    feature on top of our validated competition model.
    """
    ...
    missing_train = [c for c in required if c not in train_source.columns]
    missing_test = [c for c in required if c not in test_source.columns]
    if missing_train or missing_test:
        raise ValueError(
            f"Missing recipe columns. train={missing_train}, test={missing_test}"
        )
    ...
    def build(df: pd.DataFrame) -> np.ndarray:
        ...
        allowed_subsidy = {"yes", "no"}
        allowed_anxiety = {"low", "medium", "high"}

        if not subsidy_values.issubset(allowed_subsidy):
            raise ValueError(
                "Unexpected Subsidy_Available labels: "
                f"{sorted(subsidy_values)}"
            )

        if not anxiety_values.issubset(allowed_anxiety):
            raise ValueError(
    ...
    feature_name = "FE__original_source_recipe_score"

    X_train = X_train.copy()
    X_test = X_test.copy()

    X_train[feature_name] = build(train_source)
    X_test[feature_name] = build(test_source)
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + INCOME DIGITS + SOURCE-RECIPE SCORE EXPERIMENT")
    print("=" * 88)
    ...
    # ONLY candidate change in THIS experiment:
    # add one target-free score derived from the public ORIGINAL-dataset recipe.
    (
        X_candidate_base,
        X_candidate_test_base,
        recipe_feature,
    ) = add_source_recipe_score(
    ...
    print("HYPOTHESIS:")
    print(
        "  The public ORIGINAL-dataset buying recipe may remain a useful "
        "representation in the synthetic competition data, even though the "
        "competition generator is not assumed to be identical."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Keep the validated income-digit representation and add ONE "
        "target-free source-informed score:"
    )
    print(f"    - {recipe_feature}")
    print()
    print("HELD FIXED:")
    ...
    for outer_fold in range(5):
            ...
            importance_rows.append(
                {
                    ...
                    "is_source_recipe_feature": (
                        feature == recipe_feature
                    ),
                    "is_exact_te_feature": (
        ...
        print(
            ...
            f"+recipe={candidate_fold_auc:.8f} "
    ...
    recipe_importance.to_csv(
        args.output_dir / "source_recipe_feature_importance.csv",
        index=False,
    )
    ...
    if (
        ...
        decision = "KEEP_SOURCE_RECIPE_SCORE"
    else:
        decision = "REJECT_SOURCE_RECIPE_SCORE"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + INCOME DIGITS + SOURCE-RECIPE SCORE",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Can the public ORIGINAL-dataset buying recipe add useful structural",
        "signal to the competition model as one target-free score feature?",
        "",
        "ONLY CHANGE",
        f"Added one source-informed feature: {recipe_feature}",
        "",
        "HELD FIXED",
        "- frozen 5 folds",
        ...
        f"Income digits + recipe score OOF: {candidate_auc:.8f}",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            ...
            f"+recipe={row.digit_candidate_auc:.8f} "
    ...
    summary_lines.extend(
        [
            "",
            "SOURCE-RECIPE FEATURE IMPORTANCE",
        ]
    )

    for row in recipe_importance.itertuples():
        summary_lines.append(
            f"{row.feature}: "
    ...
    print()
    print("=" * 88)
    print("INCOME DIGITS + SOURCE-RECIPE SCORE EXPERIMENT COMPLETE")
    print("=" * 88)
    print(f"XGB m=2 baseline       : {m2_baseline_auc:.8f}")
    print(f"Income-digit baseline  : {income_digit_baseline_auc:.8f}")
    print(f"+ source recipe score  : {candidate_auc:.8f}")
    ...
    print()
    print("Source-recipe feature importance:")
    if len(recipe_importance):
        print(
            recipe_importance[
                [
                    "feature",
    ...
    print("  4. source_recipe_feature_importance.csv")
