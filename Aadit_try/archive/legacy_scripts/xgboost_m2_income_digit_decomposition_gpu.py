...
Controlled experiment — XGBoost exact-TE m=2 plus Annual_Income_USD digit decomposition.
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + INCOME DIGIT-DECOMPOSITION EXPERIMENT")
    print("=" * 88)
    ...
    # ONLY candidate representation change.
    ...
    print("HYPOTHESIS:")
    print(
        "  Explicit decimal structure in Annual_Income_USD may expose "
        "synthetic-generator signal that the raw numeric value + exact TE "
        "does not fully capture."
    )
    print()
    print("ONLY CHANGE:")
    print("  Add target-free Annual_Income_USD decimal-decomposition features:")
    ...
    for outer_fold in range(5):
        ...
        fold_rows.append(
            {
                "fold": outer_fold,
                "m2_baseline_auc": baseline_fold_auc,
        ...
        print(
            f"Fold {outer_fold}: "
            f"m2={baseline_fold_auc:.8f} -> "
            f"digits={candidate_fold_auc:.8f} "
    ...
    if (
        delta_vs_m2 > 0
        and folds_improved_vs_m2 >= 3
    ):
        decision = "KEEP_INCOME_DIGIT_REPRESENTATION"
    else:
        decision = "REJECT_INCOME_DIGIT_REPRESENTATION"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + INCOME DIGIT DECOMPOSITION",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Can explicit decimal structure in Annual_Income_USD expose additional",
        "synthetic-generator signal beyond raw income + exact-value m=2 TE?",
        "",
        "ONLY CHANGE",
        "Added target-free Annual_Income_USD digit-decomposition features:",
        *[
            f"- {feature}"
            for feature in digit_features
        ],
        ...
        f"Digit candidate OOF: {candidate_auc:.8f}",
        f"Delta vs m=2: {delta_vs_m2:+.8f}",
        f"Folds improved vs m=2: {folds_improved_vs_m2}/5",
        f"Folds worse vs m=2: {folds_worse_vs_m2}/5",
        "",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            f"Fold {row.fold}: "
            f"m2={row.m2_baseline_auc:.8f} -> "
            f"digits={row.digit_candidate_auc:.8f} "
            f"({row.delta_vs_m2:+.8f})"
        )

    summary_lines.extend(
        [
            "",
            "INCOME DIGIT FEATURE IMPORTANCE",
        ]
    )
    ...
    print()
    print("=" * 88)
    print("INCOME DIGIT-DECOMPOSITION EXPERIMENT COMPLETE")
    print("=" * 88)
    print(f"XGB m=2 baseline : {m2_baseline_auc:.8f}")
    print(f"Digit candidate  : {candidate_auc:.8f}")
    print(f"Delta vs m=2     : {delta_vs_m2:+.8f}")
    print(f"Folds improved   : {folds_improved_vs_m2}/5")
    print(f"Folds worse      : {folds_worse_vs_m2}/5")
    ...
    print("  4. income_digit_feature_importance.csv")
