...
Controlled experiment — increase only XGBoost histogram resolution from max_bin=256 to max_bin=1024 on the current learned-margin exact-frequency champion.
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_m2_exact_frequency_learned_margin_maxbin1024_gpu"
)
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + EXACT FREQUENCY + LEARNED MARGIN + MAX_BIN=1024 EXPERIMENT")
    print("=" * 88)
    ...
    print("HYPOTHESIS:")
    print(
        "  Our current model relies heavily on high-cardinality income plus exact "
        "TE/frequency representations. Increasing XGBoost histogram resolution "
        "may preserve finer numeric split structure that max_bin=256 merges."
    )
    print()
    print("ONLY CHANGE:")
    print("  XGBoost max_bin: 256 -> 1024")
    print()
    print("HELD FIXED:")
    ...
    for outer_fold in range(5):
        ...
        print(
            f"Fold {outer_fold}: "
            f"maxbin256={baseline_fold_auc:.8f} -> "
            f"maxbin1024={candidate_fold_auc:.8f} "
    ...
    if (
        ...
        decision = "KEEP_MAX_BIN_1024"
    else:
        decision = "REJECT_MAX_BIN_1024"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + EXACT FREQUENCY + LEARNED MARGIN + MAX_BIN 1024",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Does higher histogram resolution preserve useful fine-grained numeric",
        "structure in the current high-cardinality XGBoost pipeline?",
        "",
        "ONLY CHANGE",
        "XGBoost max_bin: 256 -> 1024",
        "",
        "HELD FIXED",
        "- frozen 5 folds",
        ...
        f"Learned-margin max_bin=256 baseline OOF: {learned_margin_baseline_auc:.8f}",
        f"Fixed recipe-only AUC: {recipe_only_auc:.8f}",
        f"Learned logistic prior OOF AUC: {learned_prior_auc:.8f}",
        f"Candidate max_bin=1024 OOF: {candidate_auc:.8f}",
        f"Delta vs learned-margin max_bin=256 baseline: {delta_vs_learned_margin:+.8f}",
        ...
        f"Folds improved vs learned-margin max_bin=256: {folds_improved_vs_learned_margin}/5",
        f"Folds worse vs learned-margin max_bin=256: {folds_worse_vs_learned_margin}/5",
        "",
        f"CatBoost 3-seed OOF: {catboost_auc:.8f}",
        f"Delta vs CatBoost: {delta_vs_cat:+.8f}",
        "",
        f"Probability corr vs learned-margin max_bin=256: {probability_corr_vs_learned_margin:.6f}",
        f"Rank corr vs learned-margin max_bin=256: {rank_corr_vs_learned_margin:.6f}",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            f"Fold {row.fold}: "
            f"maxbin256={row.learned_margin_baseline_auc:.8f} -> "
            f"maxbin1024={row.digit_candidate_auc:.8f} "
    ...
    print()
    print("=" * 88)
    print("LEARNED-MARGIN MAX_BIN=1024 EXPERIMENT COMPLETE")
    print("=" * 88)
    ...
    print(f"Learned-margin maxbin256: {learned_margin_baseline_auc:.8f}")
    print(f"Fixed recipe-only AUC  : {recipe_only_auc:.8f}")
    print(f"Learned prior OOF AUC  : {learned_prior_auc:.8f}")
    print(f"max_bin=1024 candidate : {candidate_auc:.8f}")
    print(f"Delta vs maxbin256     : {delta_vs_learned_margin:+.8f}")
