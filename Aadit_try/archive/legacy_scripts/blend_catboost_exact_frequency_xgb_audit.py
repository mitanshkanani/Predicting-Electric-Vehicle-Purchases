"""
Controlled blend audit: CatBoost 3-seed + exact-frequency XGBoost.

HYPOTHESIS
----------
The exact-frequency XGBoost is stronger than the recipe-margin XGBoost.
Replacing the recipe-margin XGBoost component may improve the leakage-safe
CatBoost/XGBoost rank blend.

CONTROL
-------
Before testing the new component, reproduce the current locally validated
CatBoost + recipe-margin XGBoost blend:

    meta-CV OOF = 0.94580529
    held-fold CAT weights = [0.30, 0.30, 0.28, 0.26, 0.32]
...
OLD:
    XGBoost m=2 + income digits + recipe base margin

NEW:
    XGBoost m=2 + income digits + recipe base margin
    + exact-value frequency features for income and commute
...
EXPECTED_RESTRICTED_META_AUC = 0.94580529
EXPECTED_RESTRICTED_WEIGHTS = [0.30, 0.30, 0.28, 0.26, 0.32]
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "blend_catboost_exact_frequency_xgb_audit"
)
...
def main() -> None:
    ...
    print("=" * 88)
    print("CATBOOST + EXACT-FREQUENCY XGBOOST LEAKAGE-SAFE RANK-BLEND AUDIT")
    print("=" * 88)
    print(f"Frozen fold SHA256 verified : {fold_hash}")
    print(f"CatBoost 3-seed OOF         : {cat_auc:.8f}")
    print(f"Exact-frequency XGBoost OOF : {xgb_auc:.8f}")
    print()

    # ---------------------------------------------------------
    # CONTROL: reproduce the previous 0.50 -> 1.00 audit first.
    # ---------------------------------------------------------
    print("CONTROL: reproducing current CatBoost + recipe-margin XGBoost blend...")
    ...
    restricted_folds.to_csv(
        args.output_dir
        / "recipe_margin_control_fold_metrics.csv",
        index=False,
    )
    ...
    if not (
        ...
        raise RuntimeError(
            "STOP: current recipe-margin blend control did not reproduce. "
    ...
    # ---------------------------------------------------------
    print("NEW AUDIT: CatBoost + exact-frequency XGBoost, CAT grid 0.00 -> 1.00...")
    ...
    print(
        f"Delta vs recipe-margin blend  : "
        f"{delta_vs_restricted:+.8f}"
    )
    print(
        f"Delta vs exact-frequency XGB : "
        f"{delta_vs_xgb:+.8f}"
    )
    print(
    ...
    print(
        f"Folds blend > exact-frequency XGB     : "
    ...
    expanded_folds.to_csv(
        args.output_dir
        / "exact_frequency_blend_fold_metrics.csv",
        index=False,
    )
    ...
    if (
        ...
        decision = "KEEP_EXACT_FREQUENCY_BLEND"
    elif (
        abs(
            expanded_meta_auc
            - xgb_auc
        ...
        decision = "NO_CLEAR_BLEND_GAIN_OVER_EXACT_FREQUENCY_XGB"
    elif (
        expanded_meta_auc
        <= xgb_auc
    ):
        decision = "KEEP_EXACT_FREQUENCY_XGB_SINGLE_FOR_NOW"
    else:
    ...
    summary = [
        "EXPERIMENT: CATBOOST + EXACT-FREQUENCY XGBOOST RANK-BLEND AUDIT",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Does replacing recipe-margin XGBoost with the stronger exact-frequency",
        "XGBoost improve the leakage-safe CatBoost/XGBoost rank ensemble?",
        "",
        "ONLY CHANGE",
        "XGBoost component: recipe margin -> recipe margin + exact frequencies",
        "",
        "CONTROL",
        f"Recipe-margin control meta-CV expected: {EXPECTED_RESTRICTED_META_AUC:.8f}",
        f"Recipe-margin control meta-CV reproduced: {restricted_meta_auc:.8f}",
        f"Recipe-margin control weights reproduced: {restricted_weights}",
        "CONTROL: PASS",
        "",
        "SINGLE MODELS",
        f"CatBoost 3-seed OOF: {cat_auc:.8f}",
        f"Exact-frequency XGBoost OOF: {xgb_auc:.8f}",
        "",
        "NEW META-CV BLEND",
        f"Meta-CV OOF: {expanded_meta_auc:.8f}",
        f"Delta vs current recipe-margin blend: {delta_vs_restricted:+.8f}",
        f"Delta vs exact-frequency XGB single: {delta_vs_xgb:+.8f}",
        f"Folds blend > CatBoost: {folds_blend_beats_cat}/5",
        f"Folds blend > exact-frequency XGB: {folds_blend_beats_xgb}/5",
    ...
    print("  2. summary.txt")
    print("  3. recipe_margin_control_fold_metrics.csv")
    print("  4. exact_frequency_blend_fold_metrics.csv")
