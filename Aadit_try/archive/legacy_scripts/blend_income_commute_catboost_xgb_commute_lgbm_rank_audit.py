"""
blend_income_commute_catboost_xgb_commute_lgbm_rank_audit.py

Kaggle Playground Series S6E9

Controlled ensemble update:
Replace ONLY the LightGBM member in the CURRENT validated 3-model champion.
...
hierarchical-income+commute CatBoost 3-seed
+ hierarchical-income+commute XGBoost seed 42
+ hierarchical-income+commute LightGBM
...
Run:
    python blend_income_commute_catboost_xgb_commute_lgbm_rank_audit.py
"""
...
EXPECTED_XGB_AUC = 0.94591178
EXPECTED_OLD_LGBM_AUC = 0.94578042
EXPECTED_NEW_LGBM_AUC = 0.94585287
...
OUT = (
    Path("artifacts")
    / "experiments"
    / "blend_income_commute_catboost_xgb_commute_lgbm_rank_audit"
)
...
def detect_target(train: pd.DataFrame, test: pd.DataFrame) -> str:
    ...
    if len(cols) != 1:
        raise ValueError(
            f"Target detection failed: {cols}"
        )
    return cols[0]
...
def load_oof(
    ...
    if not np.isfinite(pred).all():
        raise ValueError(
            f"Non-finite OOF predictions: {path}"
        )
...
def load_test(
    ...
    if not np.isfinite(pred).all():
        raise ValueError(
            f"Non-finite test predictions: {path}"
        )
...
def main() -> None:
    ...
    cat_rank = foldwise_rank(
        cat_oof,
        fold_ids,
    )

    xgb_rank = foldwise_rank(
        xgb_oof,
        fold_ids,
    )

    old_lgbm_rank = foldwise_rank(
        old_lgbm_oof,
        fold_ids,
    )

    new_lgbm_rank = foldwise_rank(
        new_lgbm_oof,
        fold_ids,
    )
    ...
    print(
        f"New income+commute LGBM : "
        f"{new_lgbm_auc:.8f}"
    )

    print(
        f"old LGBM vs new LGBM rank corr: "
        f"{rank_corr(old_lgbm_oof, new_lgbm_oof):.6f}"
    )

    print(
        f"new LGBM vs XGB rank corr     : "
        f"{rank_corr(new_lgbm_oof, xgb_oof):.6f}"
    )

    print(
        f"new LGBM vs CAT rank corr     : "
        f"{rank_corr(new_lgbm_oof, cat_oof):.6f}"
    )
        ...
        rows.append(
            {
                ...
                "control_xgb_weight": cx,
                "control_old_lgbm_weight": cl,
                "control_fit_auc": control_fit_auc,
                "control_held_auc": control_auc,
                "candidate_cat_weight": nc,
                "candidate_xgb_weight": nx,
                "candidate_new_lgbm_weight": nl,
    ...
    if abs(
        ...
        raise ValueError(
            "Control reproduction mismatch.\n"
            f"Loaded: {control_meta:.8f}\n"
            f"Expected: {CONTROL_META_REFERENCE:.8f}"
        )
    ...
    decision = (
        "KEEP_HIERARCHICAL_COMMUTE_LGBM_IN_3MODEL_ENSEMBLE"
        if (
            delta_meta > 0
            and improved >= 4
        )
        else
        "REJECT_HIERARCHICAL_COMMUTE_LGBM_ENSEMBLE_UPDATE"
    )
    ...
    summary = [
        "EXPERIMENT: HIERARCHICAL-COMMUTE LIGHTGBM ENSEMBLE UPDATE",
        "=" * 84,
    ...
    print("=" * 104)
    print(
        "HIERARCHICAL-COMMUTE LIGHTGBM "
        "ENSEMBLE AUDIT COMPLETE"
    )
    print("=" * 104)

    print(
        f"Control current 3-model : "
        f"{control_meta:.8f}"
    )

    print(
        f"Candidate updated model : "
        f"{candidate_meta:.8f}"
    )
