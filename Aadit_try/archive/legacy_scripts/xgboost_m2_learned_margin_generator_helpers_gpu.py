...
Controlled experiment — add four deterministic generator-oriented helper features on top of the current learned-margin exact-frequency XGBoost champion.
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_m2_learned_margin_generator_helpers_gpu"
)
...
def add_generator_helper_features(
    X_train: pd.DataFrame,
    X_test: pd.DataFrame,
    train_source: pd.DataFrame,
    test_source: pd.DataFrame,
    ...
    """
    Add four deterministic, target-free helper features.

    These are simple interaction/aggregation views of existing raw columns:
      1. ordinal range-anxiety score
      2. total nearby charging stations
      3. income × subsidy indicator
      4. environmental concern × subsidy indicator
    ...
    for column in required:
        if column not in train_source.columns or column not in test_source.columns:
            raise ValueError(
                f"Missing generator-helper source column: {column}"
            )
    ...
    def build(df: pd.DataFrame) -> pd.DataFrame:
        ...
        unexpected_anxiety = set(anxiety.unique()) - set(anxiety_map)
        if unexpected_anxiety:
            raise ValueError(
                "Unexpected Range_Anxiety_Level labels: "
                f"{sorted(unexpected_anxiety)}"
            )
        ...
        unexpected_subsidy = set(subsidy.unique()) - {"yes", "no"}
        if unexpected_subsidy:
            raise ValueError(
                "Unexpected Subsidy_Available labels: "
                f"{sorted(unexpected_subsidy)}"
            )
        ...
        return pd.DataFrame(
            {
                "FE__range_anxiety_ordinal": (
                    anxiety.map(anxiety_map)
                    .to_numpy(dtype=np.float32)
                ),
                "FE__total_charging_stations": (
                    home + work
                ).astype(np.float32),
                "FE__income_x_subsidy": (
                    income * subsidy_yes
                ).astype(np.float32),
                "FE__concern_x_subsidy": (
                    concern * subsidy_yes
                ).astype(np.float32),
            }
        )

    train_helpers = build(train_source)
    test_helpers = build(test_source)

    added = list(train_helpers.columns)

    for column in added:
        X_train[column] = train_helpers[column].to_numpy()
        X_test[column] = test_helpers[column].to_numpy()
...
def main() -> None:
    ...
    print("=" * 88)
    print("XGBOOST m=2 + LEARNED MARGIN + GENERATOR-HELPER FEATURES EXPERIMENT")
    print("=" * 88)
    ...
    # ONLY candidate feature change in THIS experiment:
    # add four deterministic generator-oriented helper features.
    (
        X_candidate_base,
        X_candidate_test_base,
        generator_helper_features,
    ) = add_generator_helper_features(
    ...
    print("HYPOTHESIS:")
    print(
        "  Simple generator-oriented aggregations/interactions may expose useful "
        "structure that depth-4 trees otherwise need several splits to recreate."
    )
    print()
    print("ONLY CHANGE:")
    print("  Add four deterministic target-free helper features:")
    for feature in generator_helper_features:
        print(f"    - {feature}")
    print()
    print("HELD FIXED:")
    ...
    for outer_fold in range(5):
            ...
            importance_rows.append(
                {
                    ...
                    "is_generator_helper_feature": (
                        feature in generator_helper_features
                    ),
                    "is_exact_te_feature": (
        ...
        print(
            ...
            f"+helpers={candidate_fold_auc:.8f} "
    ...
    helper_importance.to_csv(
        args.output_dir / "generator_helper_feature_importance.csv",
        index=False,
    )
    ...
    if (
        ...
        decision = "KEEP_GENERATOR_HELPER_FEATURES"
    else:
        decision = "REJECT_GENERATOR_HELPER_FEATURES"

    summary_lines = [
        "EXPERIMENT: XGBOOST m=2 + LEARNED MARGIN + GENERATOR HELPER FEATURES",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Can four deterministic generator-oriented helper views expose useful",
        "structure that the current depth-4 XGBoost otherwise reconstructs inefficiently?",
        "",
        "ONLY CHANGE",
        "Added: anxiety ordinal, total chargers, income×subsidy, concern×subsidy.",
        "",
        "HELD FIXED",
        "- frozen 5 folds",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            ...
            f"+helpers={row.digit_candidate_auc:.8f} "
    ...
    summary_lines.extend(
        [
            "",
            "GENERATOR HELPER FEATURE IMPORTANCE",
        ]
    )

    for row in helper_importance.itertuples():
        summary_lines.append(
            f"{row.feature}: "
    ...
    print()
    print("=" * 88)
    print("LEARNED MARGIN + GENERATOR-HELPER FEATURES EXPERIMENT COMPLETE")
    print("=" * 88)
    ...
    print(f"+ generator helpers     : {candidate_auc:.8f}")
    ...
    print()
    print("Generator-helper feature importances:")
    if len(helper_importance):
        print(
            helper_importance[
                [
                    "feature",
    ...
    print("  4. generator_helper_feature_importance.csv")
