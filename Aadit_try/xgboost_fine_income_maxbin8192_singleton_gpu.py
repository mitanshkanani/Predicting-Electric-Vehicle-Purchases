"""
xgboost_fine_income_maxbin8192_singleton_gpu.py

Kaggle Playground Series S6E9

CONTROLLED HIGH-RESOLUTION HISTOGRAM + SINGLETON SYNERGY EXPERIMENT
-----------------------------------------------
Immediate baseline:
    fine-income XGBoost with max_bin=8192 OOF = 0.94606401

HYPOTHESIS
----------
Singleton feature-tree constraints were strongly negative at max_bin=256,
while unrestricted max_bin=8192 was essentially neutral. This controlled
synergy test asks whether much higher histogram resolution changes the behavior
of singleton interaction constraints.

ONLY CHANGE RELATIVE TO THE VERIFIED max_bin=8192 BASELINE
-----------------------------------------------------------
...
NO feature changes.
NO parameter grid.
NO other model-parameter changes.

This is intentionally one controlled structural change.

Run:
    python xgboost_fine_income_maxbin8192_singleton_gpu.py
"""
...
EXPECTED_BASELINE_AUC = 0.94606401
...
DEFAULT_OUTPUT_DIR = (
    Path("artifacts")
    / "experiments"
    / "xgboost_fine_income_maxbin8192_singleton_gpu"
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Fine-income XGB max_bin=8192 + singleton interaction-constraint synergy test."
        )
    )
...
def main() -> None:
    ...
    if abs(
        ...
        raise ValueError(
            "Loaded baseline OOF does not match the verified max_bin=8192 baseline.\n"
    ...
    print("=" * 100)
    print("XGBOOST FINE-INCOME + MAX_BIN 8192 + SINGLETON CONSTRAINTS")
    print("=" * 100)
    ...
    print(
        f"Verified max_bin=8192 baseline: "
        f"{baseline_auc:.8f}"
    )
    print()
    print("HYPOTHESIS:")
    print(
        "  Test whether high histogram resolution changes the strongly negative "
        "behavior previously seen with singleton feature-tree constraints."
    )
    print()
    print("ONLY CHANGE VS VERIFIED max_bin=8192 BASELINE:")
    ...
    print("  - max_bin=8192 and all other XGBoost parameters")
    print("  - seed 42")
    print("  - CUDA GPU")
    print()
    ...
    for outer_fold in range(5):
        ...
        # Keep the verified high-resolution baseline configuration fixed.
        # The ONE candidate change is the singleton interaction constraint:
        # every tree may use only one feature.
    ...
    if (
        ...
        decision = (
            "POSITIVE_MAXBIN8192_SINGLETON_SYNERGY"
        )
    elif (
        delta_vs_baseline < 0
        or folds_worse >= 4
        ...
        decision = (
            "NEGATIVE_MAXBIN8192_SINGLETON_SYNERGY"
        )
    else:
        decision = (
            "WEAK_OR_INCONSISTENT_MAXBIN8192_SINGLETON_SYNERGY"
        )
    ...
    summary_lines = [
        "EXPERIMENT: XGBOOST FINE-INCOME + MAX_BIN 8192 + SINGLETON CONSTRAINTS",
        "=" * 84,
        "",
        "HYPOTHESIS",
        "Can high histogram resolution rescue the strongly negative singleton",
        "feature-tree behavior previously observed at max_bin=256?",
        "",
        "ONLY CHANGE VS VERIFIED max_bin=8192 BASELINE",
        "Add one singleton interaction-constraint group per feature.",
        "",
        "HELD FIXED",
        ...
        "",
        "RESULTS",
        f"Verified max_bin=8192 baseline OOF: {baseline_auc:.8f}",
        f"max_bin=8192 + singleton candidate OOF: {candidate_auc:.8f}",
    ...
    print()
    print("=" * 96)
    print("MAX_BIN 8192 + SINGLETON SYNERGY EXPERIMENT COMPLETE")
    print("=" * 96)
    print(f"max_bin=8192 baseline: {baseline_auc:.8f}")
