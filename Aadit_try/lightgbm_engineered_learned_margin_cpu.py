...
Controlled experiment — switch only the boosting model family to LightGBM while reusing the validated engineered representation, leakage-safe TE, and nested learned logistic prior.
...
DEFAULT_LEARNED_MARGIN_XGB_OOF = (
    Path("artifacts")
    / "experiments"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--learned-margin-xgb-oof",
        type=Path,
        default=DEFAULT_LEARNED_MARGIN_XGB_OOF,
    )
...
def build_model() -> lgb.LGBMClassifier:
    """
    Fixed, conservative LightGBM configuration.

    This experiment tests the model family / leaf-wise tree geometry on the
    already-validated feature representation. These parameters are not tuned
    in this experiment.
    """
...
def main() -> None:
    ...
    print("=" * 88)
    print("LIGHTGBM + VALIDATED ENGINEERED REPRESENTATION + LEARNED MARGIN")
    print("=" * 88)
    ...
    # Fixed source recipe retained only for diagnostics/reference.
    (
        train_recipe_margin,
        train_recipe_score,
    ...
    learned_margin_xgb_oof = load_oof(
        args.learned_margin_xgb_oof,
        train,
        fold_ids,
        id_col,
    ...
    learned_margin_xgb_auc = float(
        roc_auc_score(
            y,
            learned_margin_xgb_oof,
        )
    )
    ...
    print(f"Current XGB champion OOF : {learned_margin_xgb_auc:.8f}")
    print(f"Fixed recipe-only AUC    : {recipe_only_auc:.8f}")
    print()
    print("HYPOTHESIS:")
    print(
        "  Raw LightGBM was weak earlier, but the representation has changed "
        "substantially since then. Leaf-wise LightGBM may exploit the validated "
        "income digits, exact TE, frequency features, and learned prior "
        "differently enough to become a strong and useful diverse model."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Model family: XGBoost -> LightGBM. The validated feature matrix, "
        "nested TE, learned logistic prior, frozen folds, and seed remain fixed."
    )
    print()
    print("HELD FIXED:")
    ...
    print("  - smoothing m=2")
    print("  - all validated feature engineering")
    print("  - nested learned logistic base margin")
    print("  - seed 42")
    print()
    print("FIXED LIGHTGBM CONFIG (not tuned here):")
    print("  - num_leaves=31")
    print("  - min_child_samples=50")
    print("  - learning_rate=0.03")
    print("  - subsample=0.90")
    print("  - colsample_bytree=0.90")
    print("  - reg_lambda=2.0")
    print("  - CPU device (installed build)")
    print()
    ...
    for outer_fold in range(5):
        ...
        baseline_fold_auc = float(
            roc_auc_score(
                y[valid_idx],
                learned_margin_xgb_oof[valid_idx],
            )
        )
        ...
        fold_rows.append(
            {
                "fold": outer_fold,
                "xgb_champion_auc": baseline_fold_auc,
                "digit_candidate_auc": candidate_fold_auc,
                "delta_vs_xgb_champion": (
                    candidate_fold_auc
                    - baseline_fold_auc
                ),
        ...
        print(
            f"Fold {outer_fold}: "
            f"xgb_champion={baseline_fold_auc:.8f} -> "
            f"lightgbm={candidate_fold_auc:.8f} "
    ...
    probability_corr_vs_xgb_champion = float(
        np.corrcoef(
            oof,
            learned_margin_xgb_oof,
        )[0, 1]
    ...
    rank_corr_vs_xgb_champion = rank_corr(
        oof,
        learned_margin_xgb_oof,
    )
    ...
    frequency_importance.to_csv(
        args.output_dir / "exact_frequency_feature_importance.csv",
        index=False,
    )
    ...
    if (
        delta_vs_xgb_champion > 0
        and folds_improved_vs_xgb >= 3
    ):
        decision = "NEW_SINGLE_MODEL_CHAMPION"
    elif (
        candidate_auc > catboost_auc
        and rank_corr_vs_xgb_champion < 0.9995
    ):
        decision = "KEEP_LIGHTGBM_FOR_DIVERSITY_AUDIT"
    else:
        decision = "REJECT_ENGINEERED_LIGHTGBM"

    summary_lines = [
        "EXPERIMENT: LIGHTGBM + VALIDATED ENGINEERED REPRESENTATION + LEARNED MARGIN",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "Can LightGBM's leaf-wise growth exploit the already-validated engineered",
        "representation strongly enough to become a competitive/diverse model?",
        "",
        "ONLY CHANGE",
        "Model family: XGBoost -> LightGBM.",
        "",
        "HELD FIXED",
        "- frozen 5 folds",
        ...
        f"Exact-frequency XGB OOF: {exact_frequency_baseline_auc:.8f}",
        f"Current learned-margin XGB champion OOF: {learned_margin_xgb_auc:.8f}",
        f"Fixed recipe-only AUC: {recipe_only_auc:.8f}",
        f"Learned logistic prior OOF AUC: {learned_prior_auc:.8f}",
        f"Engineered LightGBM OOF: {candidate_auc:.8f}",
        f"Delta vs current XGB champion: {delta_vs_xgb_champion:+.8f}",
        f"Delta vs exact-frequency XGB: {delta_vs_exact_frequency:+.8f}",
        ...
        f"Folds improved vs current XGB champion: {folds_improved_vs_xgb}/5",
        f"Folds worse vs current XGB champion: {folds_worse_vs_xgb}/5",
        "",
        f"CatBoost 3-seed OOF: {catboost_auc:.8f}",
        f"Delta vs CatBoost: {delta_vs_cat:+.8f}",
        "",
        f"Probability corr vs current XGB champion: {probability_corr_vs_xgb_champion:.6f}",
        f"Rank corr vs current XGB champion: {rank_corr_vs_xgb_champion:.6f}",
    ...
    for row in fold_metrics.itertuples():
        summary_lines.append(
            f"Fold {row.fold}: "
            f"exact_frequency={row.xgb_champion_auc:.8f} -> "
            f"+learned_margin={row.digit_candidate_auc:.8f} "
            f"({row.delta_vs_xgb_champion:+.8f})"
        )

    summary_lines.extend(
        [
            "",
            "EXACT-FREQUENCY FEATURE IMPORTANCE (LIGHTGBM MODEL)",
        ]
    )

    for row in frequency_importance.itertuples():
        summary_lines.append(
            f"{row.feature}: "
    ...
    print()
    print("=" * 88)
    print("ENGINEERED LIGHTGBM EXPERIMENT COMPLETE")
    print("=" * 88)
    ...
    print(f"Exact-frequency XGB     : {exact_frequency_baseline_auc:.8f}")
    print(f"Current XGB champion    : {learned_margin_xgb_auc:.8f}")
    print(f"Fixed recipe-only AUC   : {recipe_only_auc:.8f}")
    print(f"Learned prior OOF AUC   : {learned_prior_auc:.8f}")
    print(f"Engineered LightGBM     : {candidate_auc:.8f}")
    print(f"Delta vs XGB champion   : {delta_vs_xgb_champion:+.8f}")
    print(f"Delta vs exact freq XGB : {delta_vs_exact_frequency:+.8f}")
    ...
    print(f"Folds improved vs XGB   : {folds_improved_vs_xgb}/5")
    print(f"Folds worse vs XGB      : {folds_worse_vs_xgb}/5")
    print(f"Rank corr vs XGB        : {rank_corr_vs_xgb_champion:.6f}")
    print(f"Decision         : {decision}")
    print(f"Runtime          : {total_seconds:.2f}s")
    print(f"Artifacts        : {args.output_dir.resolve()}")
    print()
    print("Exact-frequency feature importances:")
    if len(frequency_importance):
        print(
            frequency_importance[
                [
                    "feature",
