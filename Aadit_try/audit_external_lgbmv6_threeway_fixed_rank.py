"""
audit_external_lgbmv6_threeway_fixed_rank.py
...
LGBMV6_OOF_PATH = (
    ROOT / "external_oof_strong"
    / "Pure LGBM_V6_oof.csv"
)

LGBMV6_TEST_PATH = (
    ROOT / "external_oof_strong"
    / "Pure LGBM_V6_test.csv"
)

OUTPUT_DIR = (
    ROOT / "artifacts" / "experiments"
    / "external_lgbmv6_threeway_fixed_rank_audit"
)
...
EXPECTED_LGBMV6_AUC = 0.94606774
...
def prediction_column(df: pd.DataFrame, id_col: str | None) -> str:
    ...
    raise RuntimeError(
        f"Could not identify unambiguous prediction column. "
        f"Columns={list(df.columns)}, valid={valid}"
    )
...
def load_internal_oof(
    ...
    p = pd.to_numeric(df[col], errors="raise").to_numpy(dtype=np.float64)
...
def load_id_aligned(
    ...
    if id_col not in df.columns:
        raise RuntimeError(
            f"{path.name} missing required ID column {id_col!r}"
        )
...
def main() -> None:
    ...
    checks = [
        ...
        ("external_lgbmv6", lgbm_auc, EXPECTED_LGBMV6_AUC),
    ]
    ...
    for label, found, expected in checks:
        if abs(found - expected) > AUC_TOL:
            raise RuntimeError(
                ...
                f"expected {expected:.8f}, found {found:.8f}"
            )
    ...
    baseline_oof = (r_internal + r_xgb10) / 2.0
    candidate_oof = (
    ...
    if delta >= 3e-5 and folds_improved >= 4:
        primitive = "POSITIVE_LGBMV6_THREE_WAY_SIGNAL"
    elif delta <= 0 or folds_worse >= 4:
        primitive = "NEGATIVE_LGBMV6_THREE_WAY_SIGNAL"
    else:
        primitive = "WEAK_OR_INCONSISTENT_LGBMV6_THREE_WAY_SIGNAL"

    # Exact deployment counterpart.
    candidate_test = (
        rank_pct(internal_test)
        + rank_pct(xgb10_test)
        + rank_pct(lgbm_test)
    ) / 3.0
        ...
        [
            {
                ...
                "external_lgbmv6_auc": lgbm_auc,
                "two_way_champion_auc": baseline_auc,
                "three_way_auc": candidate_auc,
                "delta_vs_two_way_champion": delta,
                "rank_corr_lgbmv6_vs_two_way": corr_lgbm_baseline,
                "rank_corr_lgbmv6_vs_internal": corr_lgbm_internal,
                "rank_corr_lgbmv6_vs_xgb10": corr_lgbm_xgb10,
    ...
    summary = [
        "EXPERIMENT: FIXED THREE-WAY RANK BLEND WITH EXTERNAL LGBM V6",
        "=" * 92,
        "",
        "TYPE",
        ...
        "HYPOTHESIS",
        (
            "Can the lower-correlation Pure LGBM V6 add useful diversity "
            "beyond the current internal+XGB10 champion?"
        ),
        "",
        "ONLY CHANGE",
        (
            "Current 50/50 internal+XGB10 rank average -> "
            "fixed equal 1/3 internal + 1/3 XGB10 + 1/3 LGBM V6."
        ),
        "",
        "NO WEIGHT SEARCH",
        ...
        f"LGBM V6 OOF: {lgbm_oof_col}",
        f"LGBM V6 test: {lgbm_test_col}",
        "",
        "STANDALONE OOF",
        f"Internal champion: {internal_auc:.8f}",
        f"External XGB10: {xgb10_auc:.8f}",
        f"External LGBM V6: {lgbm_auc:.8f}",
        "",
        "ENSEMBLE RESULT",
        ...
        f"Rank corr LGBM V6 vs 2-way: {corr_lgbm_baseline:.6f}",
        f"Rank corr LGBM V6 vs internal: {corr_lgbm_internal:.6f}",
        f"Rank corr LGBM V6 vs XGB10: {corr_lgbm_xgb10:.6f}",
    ...
    print("=" * 108)
    print("FIXED THREE-WAY RANK-BLEND AUDIT — EXTERNAL LGBM V6")
    print("=" * 108)
    print(f"Frozen fold SHA256 : {fold_hash}")
    print(f"Internal OOF       : {internal_auc:.8f}")
    print(f"External XGB10 OOF : {xgb10_auc:.8f}")
    print(f"External LGBM V6   : {lgbm_auc:.8f}")
    print()
    print(f"Current 2-way      : {baseline_auc:.8f}")
    print(f"Fixed 3-way        : {candidate_auc:.8f}")
    print(f"Delta              : {delta:+.8f}")
    print()
    print(f"LGBM V6 rank corr vs 2-way : {corr_lgbm_baseline:.6f}")
    print(f"LGBM V6 rank corr vs int   : {corr_lgbm_internal:.6f}")
    print(f"LGBM V6 rank corr vs XGB10 : {corr_lgbm_xgb10:.6f}")
    print()
