...
Add fine-grained Annual_Income_USD target encodings at $50 and $250 scales
on top of the CURRENT hierarchical-income + hierarchical-commute XGBoost
champion.

HYPOTHESIS
----------
The current model already contains:
- exact income Bayesian TE
- income digit decomposition
- hierarchical income TE at $1k / $10k / $100k

The missing representation scale is local income neighbourhood structure between
exact identity and the existing $1k bucket. Fine $50 and $250 buckets may share
signal across nearby income values without collapsing a full $1k range.

ONLY CHANGE
-----------
Add two nested leakage-safe income Bayesian target encodings:

    HTE__income_50_bucket
    HTE__income_250_bucket

where:
    $50 bucket  = floor(Annual_Income_USD / 50)
    $250 bucket = floor(Annual_Income_USD / 250)

Both use the already-frozen Bayesian smoothing:
    m = 2
...
--------------
For each outer fold:
- outer-training rows receive inner-OOF fine-income encodings using only other
...
BASELINE
--------
Current hierarchical-income + hierarchical-commute XGB champion:
    OOF AUC = 0.94591178

Run:
    python xgboost_fine_income_te_gpu.py
"""
...
def main() -> None:
    ...
    print("=" * 96)
    print("XGBOOST + FINE-GRAINED INCOME TARGET ENCODING")
    print("=" * 96)
    ...
    print(
        f"Current hierarchical-income + commute XGB baseline: "
        f"{baseline_auc:.8f}"
    )
    print()
    print("HYPOTHESIS:")
    print(
        "  Existing income representation jumps from exact identity to $1k buckets. "
        "$50/$250 neighbourhoods may capture additional local income structure."
    )
    print()
    print("ONLY CHANGE:")
    print(
        "  Add nested leakage-safe income TEs at $50 and $250 scales."
    )
    print()
    print("HELD FIXED:")
    ...
    summary_lines = [
        "EXPERIMENT: XGBOOST + FINE-GRAINED INCOME TARGET ENCODING",
        "=" * 84,
        "",
        "HYPOTHESIS",
        ...
        "",
        "ONLY CHANGE",
        "Add nested leakage-safe income TE at $50 / $250 scales.",
        "",
        "HELD FIXED",
        ...
        "",
        "RESULTS",
        f"Current hierarchical-income + commute XGB baseline OOF: {baseline_auc:.8f}",
        f"Fine-income candidate OOF: {candidate_auc:.8f}",
    ...
    print()
    print("=" * 96)
    print("FINE-GRAINED INCOME TE EXPERIMENT COMPLETE")
    print("=" * 96)
