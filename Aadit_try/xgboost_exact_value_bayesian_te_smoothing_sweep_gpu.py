...
Controlled sweep — Exact-value Bayesian target-encoding smoothing for XGBoost GPU.
...
SEED = 42
SMOOTHING = 100.0
SMOOTHING_GRID = (25.0, 50.0, 100.0, 200.0, 400.0)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=(
            Path("artifacts")
            / "experiments"
            / "xgboost_exact_value_bayesian_te_smoothing_sweep_gpu"
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Shared Bayesian smoothing strengths to test. Default: 25 50 100 200 400",
    )
...
def main() -> None:
    ...
    control_candidates = [
        r for r in candidate_results if np.isclose(r["smoothing"], 100.0)
    ]
    if not control_candidates:
        raise ValueError(
            "The default experiment requires m=100 in the grid as the reproduced control."
        )
        ...
        sweep_rows.append(
            {
                ...
                "delta_vs_m100": float(result["oof_auc"]) - control_auc,
                "folds_improved_vs_m100": int(
                    sum(delta > 0 for delta in per_fold_vs_control)
                ),
                "folds_worse_vs_m100": int(
                    sum(delta < 0 for delta in per_fold_vs_control)
                ),
                "probability_corr_vs_catboost": float(
    ...
    if np.isclose(best_smoothing, 100.0):
        ...
        and int(best_row["folds_improved_vs_m100"]) >= 3
    ):
        decision = "CANDIDATE_SMOOTHING_IMPROVEMENT"
    else:
        decision = "KEEP_M100"
    ...
    # Reproduction sanity check for our historical m=100 result.
    historical_m100_auc = 0.94441818
    m100_reproduction_gap = control_auc - historical_m100_auc

    summary_lines = [
        "EXPERIMENT: XGBOOST EXACT-VALUE BAYESIAN TE SMOOTHING SWEEP",
        "=" * 78,
        "",
        "HYPOTHESIS",
        "The exact-value TE signal may improve if Bayesian shrinkage strength",
        "is better matched to the repeated exact income/commute identities.",
        "",
        ...
        f"Historical m=100 exact-TE XGB OOF: {historical_m100_auc:.8f}",
        "",
        "M=100 REPRODUCTION",
        f"Reproduced m=100 OOF: {control_auc:.8f}",
        f"Gap vs historical m=100: {m100_reproduction_gap:+.8f}",
        "",
        "SWEEP RESULTS",
    ]
    ...
    for row in sweep_results.itertuples():
        summary_lines.append(
            ...
            f"delta_vs_m100={row.delta_vs_m100:+.8f} | "
            f"folds_improved={row.folds_improved_vs_m100}/5 | "
    ...
    summary_lines.extend(
        [
            ...
            f"Delta vs reproduced m=100: {best_delta_vs_control:+.8f}",
            f"Decision: {decision}",
            "",
            ...
            "improvement is consistent enough locally to justify replacing m=100.",
        ]
    )
    ...
    print()
    print(f"Reproduced m=100 OOF : {control_auc:.8f}")
    print(f"Historical m=100 OOF : {historical_m100_auc:.8f}")
    print(f"Reproduction gap      : {m100_reproduction_gap:+.8f}")
    print()
    print(f"Best smoothing        : {best_smoothing:g}")
    print(f"Best OOF              : {best_auc:.8f}")
    print(f"Delta vs m=100        : {best_delta_vs_control:+.8f}")
