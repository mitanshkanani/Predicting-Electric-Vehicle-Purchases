"""
Predicting Electric Vehicle Purchases — Kaggle Playground S06E09  (v3)
Overfitting-diagnosis experiment. v1 and v2 both scored EXACTLY 0.94540 on the
LB even though OOF rose 0.9469 -> 0.9478: our OOF gains were NOT transferring.
Two suspects: (a) AUC-optimized blend weights tuned on the OOF (overfit by
construction), and (b) low-smoothing exact-value target encodings memorizing
train rows. v3 changes both so the LB tells us the truth:
  * REGULARIZED target encoding (smoothing 10 & 50 instead of 2 & 10).
  * SUBMIT the robust EQUAL-weight rank blend (optimizer printed for info only).
  * Versioned output file: submission_v3.csv (no more ambiguous submission.csv).
  * Same 5 diverse models, both T4s via CatBoost multi-GPU.
Judge v3 by its LB score, NOT its (now-lower) OOF.

Run as a script on Kaggle (Save & Run All). Needs only train.csv + test.csv.
"""
...
N_SPLITS = 10
RANDOM_STATE = 42
# v3: REGULARIZED target encoding (higher smoothing) to test whether the
# low-smoothing exact-value TE was overfitting train->test (OOF up, LB flat).
TE_SMOOTHINGS = (10.0, 50.0)
