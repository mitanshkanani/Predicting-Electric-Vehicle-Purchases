...
# XGBoost deliberately uses plain device="cuda" (NOT "cuda:0"/"cuda:1"). Pinning
# per-spec GPUs in the same process that also holds a CatBoost multi-GPU context
# aborted the kernel with `thrust::system_error` (uncatchable C++ abort) at ~1.8h.
XGB_USE_GPU = True
...
def init_run_state(train, test, y, out_dir, feature_cols):
    ...
    _RUN["test"] = test
    _RUN["y"] = y
    _RUN["n_train"] = len(train)
    _RUN["n_test"] = len(test)
    if RESUME:
        done = set()
        for d in {_RUN["read_dir"], _RUN["write_dir"]}:
            try:
                done |= {f[:-8] for f in os.listdir(d) if f.endswith("_oof.npy")}
            except OSError:
                pass
...
def _ckpt_files(key, d):
    return os.path.join(d, f"{key}_oof.npy"), os.path.join(d, f"{key}_test.npy")


def save_ckpt(key, oof, te):
    if not _RUN["write_dir"]:
        return
    try:
        fo, ft = _ckpt_files(key, _RUN["write_dir"])
        np.save(fo, np.asarray(oof, dtype=np.float32))
        np.save(ft, np.asarray(te, dtype=np.float32))
        log(f"[ckpt] saved {key}")
    except Exception as exc:
        log(f"[ckpt] save {key} failed: {exc!r}")


def load_ckpt(key):
    if not (RESUME and _RUN["write_dir"]):
        return None
    for d in (_RUN["read_dir"], _RUN["write_dir"]):
        fo, ft = _ckpt_files(key, d)
        if not (os.path.isfile(fo) and os.path.isfile(ft)):
            continue
        try:
            oof = np.load(fo)
            te = np.load(ft)
            ...
            continue
        if len(oof) != _RUN["n_train"] or len(te) != _RUN["n_test"]:
            log(f"[ckpt] {key} is stale (row count changed) -> retraining")
            continue
        if np.isnan(oof.astype(np.float64)).any():
            log(f"[ckpt] {key} has NaNs -> retraining")
            continue
        auc = roc_auc_score(_RUN["y"], oof)
        log(f"[ckpt] resumed {key} OOF AUC = {auc:.8f} (skipped retraining)")
        return oof, te, auc
    return None


def write_partial(results):
    """Always leave a submittable file on disk, even mid-run."""
    if not (_RUN["write_dir"] and results):
        return
    try:
        keys = list(results)
        blend_te = np.mean([rank_pct(results[k][1]) for k in keys], axis=0)
        best = max(keys, key=lambda k: results[k][2])
        probs = finalize_test_probs(blend_te, results[best][1], results[best][1], _RUN["test"])
        out = os.path.join(_RUN["out_dir"], f"submission_{VERSION}_partial.csv")
        pd.DataFrame({ID_COL: _RUN["test"][ID_COL].to_numpy(), TARGET: probs}).to_csv(out, index=False)
        log(f"[partial] wrote {os.path.basename(out)} from {len(keys)} model(s): {keys}")
    except Exception as exc:
        log(f"[partial] write failed: {exc!r}")


def run_lane(specs, X, Xte, y, fold_ids, n_gpus, results, cfg_extra=None):
    """Run a list of specs (one compute lane). Each finished model is
    checkpointed, and a spec that raises is skipped instead of killing the run."""
    for key, kind, seeds in specs:
        ...
        if kind == "catboost":
            ...
            cfg["device"] = "cuda" if (XGB_USE_GPU and n_gpus > 0) else "cpu"
        use_gpu = kind in GPU_KINDS and n_gpus > 0

        try:
            res = run_spec_sequential(key, kind, seeds, X, Xte, y, fold_ids, use_gpu, cfg)
        except Exception as exc:
            log(f"!! {key} FAILED ({exc!r}) -> continuing without this model")
            continue
        with _LOG_LOCK:
            results[key] = res
        save_ckpt(key, res[0], res[1])
...
def main():
    ...
    # ---- NN diversity model (never kills the run; checkpointed like the rest) ----
    nn_ok = False
    if USE_NN:
        cached = load_ckpt("nn")
        if cached is not None:
