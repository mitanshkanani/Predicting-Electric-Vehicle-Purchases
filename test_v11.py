"""Local verification for ev_s6e9_v11.py. LightGBM/XGBoost are not installed here so
...
# ============================================================================
work = os.path.join(HERE, "_v11_smoke")
...
files = sorted(f for f in os.listdir(work) if f.startswith("submission_v11"))
...
ck = [d for d in os.listdir(work) if d.startswith("v11_ckpt")]
...
if ck:
    ...
    got_oof = np.load(os.path.join(work, "v11_lgbm_ref_oof_rank.npy"))
    got_te = np.load(os.path.join(work, "v11_lgbm_ref_test.npy"))
