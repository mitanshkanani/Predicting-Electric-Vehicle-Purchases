"""
Predicting Electric Vehicle Purchases — Kaggle Playground S06E09  (v12)

v12 = v11 with ONE change: the full 69-anchor set is reverted to v8's 13. Everything else is
byte-for-byte v11 — same 5-repeat bagging, same fold-rank OOF discipline, same bands, same
members, same gate.

WHY THE REVERSION. The anchor change was v11's headline idea: najiama's real kernel source
maps the external 10k dataset's per-column target mean onto every categorical, numeric AND
digit column (~66 `_org_mean` features) while v8 built only 13. On a toy 13-raw-column
CatBoost that measured +0.000225 held out, so v11 shipped it. It did not transfer. Re-run
against v11's OWN `build_features`/`fold_matrix` on a 50/50 split
(`research/spike2_faithful_ablation.py`), the full anchor set is **-0.000019** — dead. The
toy proxy misled: a feature that adds information to a 13-column model adds nothing to a
291-column design that already target-encodes the same keys at three smoothings. Reverting
costs nothing, removes 20 columns that can only overfit, and runs ~15% faster.

WHAT ACTUALLY CARRIES v11, and it is worth stating plainly because it changes what to do next.
The same faithful ablation measured v11's other change at **+0.000347** for 3-seed bagging
(vs the MEAN of three single seeds, not the luckiest baseline; seed spread sd 0.000048, so
that is 7.3 sigma). Fitting rho + (1-rho)/K gives inter-seed rho = 0.99948 and a predicted
+0.000416 at the K=5 v12 already runs, saturating at +0.000494. **v12's whole expected edge
over v8 is bagging, not features.**

MEASURED EXHAUSTED — do not re-attempt any of these; full evidence in
FINDINGS_S6E9_reverse_engineering.md:
  - Hardcoding / reverse-engineering the labels. 15 rule families tested cross-fitted, zero
    survived; best delta +0.000019 with impurities inside its own "certain" set. The 24+2
    bands already shipped score 6,118 held-out rows with ZERO impurities and +0.000422 —
    the determinism is already fully collected. No duplicate rows (0 of 286,571).
  - Blending more of this model family. All 376 pairs among the 28 OOF vectors >= 0.944 in
    this repo: median gain by rho bucket is -0.000013 to +0.000026; best pair out of 376
    reached +0.000225, which is a max over 376 tries, not an expectation. Leave-one-fold-out
    Caruana over the top 40 of 117 vectors gives an honest 0.946133 — +0.000078 over v8.
  - $50/$250 fine-income bins: +0.000027, neutral. KEPT anyway; it is free and two
    independent measurements lean positive.
  - Past 5 repeats: 10 buys a predicted +0.00005 for double runtime. Not worth it.

HONEST STATE OF PLAY. v11 scored 0.94620, only +0.00003 over v8, when bagging alone predicts
~+0.0004. With a public-LB standard error of ~0.0013 that is a 0.28-sigma miss — ordinary
noise. The likeliest reading is that v11/v12 genuinely beat v8 and we drew badly on the public
split. The gap to Aadit's 0.94643 is +0.00023 = 18% of one standard error, and there is no
evidence his candidate is a better model. Nothing left on this list has an expected value
above +0.00005. Run v12, keep it alongside v11 as the two slots, and if Aadit's submission
correlates with v11 below ~0.990 use `make_v12_blend.py` — above that, blending is a coin flip.

KAGGLE: one Input (train/test/sample_submission + EV_Adoption CSV), GPU on. Runtime ~45-60 min.
Outputs: submission_v12.csv (gate's pick), submission_v12_single.csv (best member),
v12_ckpt_*/, v12_*.npy for offline audits.
"""
...
FULL_ANCHOR_SET = False       # change C1: anchors for categoricals + numerics + DIGITS
ORIG_AS_ROWS = False
...
def main():
    ...
    for m in live:
        np.save(os.path.join(_RUN["out_dir"], f"v12_{m}_oof_rank.npy"), oof_rnk[m])
        np.save(os.path.join(_RUN["out_dir"], f"v12_{m}_oof_raw.npy"), oof_raw[m])
        np.save(os.path.join(_RUN["out_dir"], f"v12_{m}_test.npy"), te_rnk[m])
