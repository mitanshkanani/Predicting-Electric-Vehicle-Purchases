"""
Predicting Electric Vehicle Purchases — Kaggle Playground S06E09  (v6)
Our own build, informed by studying the public winners (not copied). What we
learned: the top public "Pure LGBM" hits LB 0.94638 in 23 min as a SINGLE
well-tuned LightGBM, and it uses the original "EV Adoption Behavior and Range
Anxiety" dataset as an input. Our v1-v5 sat at ~0.9454 because our exact-value
target encoding inflated CV but did NOT transfer to LB.

v6 changes:
  * Adds the original dataset to training when present (auto-detected in
    /kaggle/input) — the winners use it.
  * Drops the CV-inflating EXACT-value income/commute TE; keeps only generalizable
    bucketed TE + engineered features (income digits/buckets/ratios/recipe).
  * Clean, well-tuned LGBM-led ensemble (LGBM x2, XGB x2, CatBoost), rank-averaged.
  * Both T4s via CatBoost multi-GPU. Output: submission_v6.csv.

To use the original data: in the Kaggle notebook, Add Input -> dataset
"itzzomkar/ev-adoption-behavior-and-range-anxiety". v6 auto-detects and merges it.
"""
...
N_SPLITS = 10
RANDOM_STATE = 42
# v6: bucketed-only TE at moderate smoothing (exact-value TE removed below).
TE_SMOOTHINGS = (10.0, 30.0)
# Merge the original source dataset into training if it's attached as input.
...
# Each spec: (family_key, model_kind, seeds). Multiple specs of the same kind
# with different params add diversity for the blend.
MODEL_SPECS = [
    ("cat",     "catboost", (0, 1)),
    ("xgb_a",   "xgboost",  (42, 7)),
    ("xgb_b",   "xgboost",  (13,)),
...
# v6: BUCKETED target encoding only (generalizes). The exact-value income/commute
# TE inflated our CV (OOF 0.947) but did NOT transfer to LB (0.9454), so it's out.
TE_KEYS = {
    "te_incb_5k": ["inc_b_5k"],
    "te_incb_1k": ["inc_b_1k"],
    "te_cmb": ["comm_b5"],
...
# ----------------------------------------------------------------------------
# Original source dataset (auto-detected under /kaggle/input).
# ----------------------------------------------------------------------------
def load_original(train, skip_paths):
    """Find the 'EV Adoption Behavior' source CSV (same feature names) and
    return it aligned to the competition schema, or None."""
...
def main():
    ...
    results = {}
    gpu_specs = [s for s in MODEL_SPECS if s[1] in GPU_KINDS]
    cpu_specs = [s for s in MODEL_SPECS if s[1] not in GPU_KINDS]

    if parallel:
        try:
            results.update(run_specs_parallel(gpu_specs, Xtr[feature_cols], Xte[feature_cols], y, fold_ids, n_gpus))
        except (Exception, BrokenProcessPool) as exc:
            print(f"[parallel] FAILED ({exc!r}) -> sequential GPU fallback", flush=True)
            results = {}
    ...
    # ---- Blending ----
    # v3: SUBMIT the robust EQUAL-weight rank average. The OOF-optimized weights
    # are printed for info only — they overfit the CV (that's why OOF rose while
    # the real LB stayed flat). We trust the honest, low-variance blend instead.
    oof_dict = {k: v[0] for k, v in results.items()}
    opt_w, opt_auc = optimize_weights(oof_dict, y)

    # Equal-weight rank blend (used for the submission).
    eq_oof = np.mean([rank_pct(results[k][0]) for k in results], axis=0)
    eq_te = np.mean([rank_pct(results[k][1]) for k in results], axis=0)
    eq_auc = roc_auc_score(y, eq_oof)

    print("\n" + "=" * 70)
    for k, (_o, _t, auc) in results.items():
        print(f"{k:8s} OOF AUC = {auc:.8f}   (opt weight {opt_w[k]:.3f})")
    print(f"{'EQUAL':8s} OOF AUC = {eq_auc:.8f}   <-- SUBMITTED")
    print(f"{'OPT':8s} OOF AUC = {opt_auc:.8f}   (info only, overfits OOF)")
    print("=" * 70, flush=True)

    # Map the equal-weight test blend ranks onto the best model's probability
    # distribution (AUC is rank-invariant, so ordering is what matters).
    best_key = max(results, key=lambda k: results[k][2])
    ref_sorted = np.sort(results[best_key][1])
    ranks = np.argsort(np.argsort(eq_te))
    idx = (ranks / (len(eq_te) - 1) * (len(ref_sorted) - 1)).astype(int)
