"""
Predicting Electric Vehicle Purchases — Kaggle Playground S06E09  (v2)
Stronger self-contained blend: multi-GPU + AUC-optimized weighted blend + more
diverse models + richer target encodings.

Why v2 should beat 0.94675:
  * Overnight run showed LightGBM ALONE = 0.94719 OOF > equal-weight blend
    0.94692 > LB best 0.94675. Equal averaging let weak CatBoost drag the blend
    down. v2 replaces it with AUC-OPTIMIZED non-negative blend weights.
  * Adds a 2nd LightGBM + 2nd XGBoost config for diversity, and more
    interaction target-encodings (income x car/gender/anxiety, commute x city).
  * Uses BOTH T4 GPUs: CatBoost trains one model across both cards, and the
    GPU models run their CV folds in parallel across two GPU-pinned worker
    processes. A robust sequential fallback keeps the run alive if the pool
    ever fails.
...
def main():
    ...
    # ---- AUC-optimized weighted blend ----
    oof_dict = {k: v[0] for k, v in results.items()}
    weights, blend_auc = optimize_weights(oof_dict, y)

    print("\n" + "=" * 70)
    for k, (_o, _t, auc) in results.items():
        print(f"{k:8s} OOF AUC = {auc:.8f}   weight = {weights[k]:.3f}")
    # Equal-weight comparison.
    eq = np.zeros(len(y))
    for k in results:
        eq += rank_pct(results[k][0])
    eq /= len(results)
    print(f"{'EQUAL':8s} OOF AUC = {roc_auc_score(y, eq):.8f}")
    print(f"{'OPT':8s} OOF AUC = {blend_auc:.8f}")
    print("=" * 70, flush=True)

    # Apply weights to test predictions (rank blend), map to calibrated probs.
    te_rank = np.zeros(len(Xte))
    for k, w in weights.items():
        te_rank += w * rank_pct(results[k][1])
    # Use the highest-weight model's test distribution as the probability reference.
    ref_key = max(weights, key=weights.get)
    ref_sorted = np.sort(results[ref_key][1])
